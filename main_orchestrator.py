...
def save_daily_report(data):
    ...
    with open(report_file, "w", encoding="utf-8") as f:
        ...
        # UX: Visual Status Indicators
        site_s = status.get('site_status')
        site_icon = "✅" if site_s == "healthy" else "❌"

        robots_s = status.get('robots_txt_accessible')
        robots_icon = "✅ Allowed" if robots_s else "❌ Blocked"

        google_s = status.get('googlebot_allowed')
        google_icon = "✅ Allowed" if google_s else "❌ Blocked"

        f.write(f"- **Site Status:** {site_icon} {site_s} (Code: {status.get('site_code')})\n")
        f.write(f"- **Robots.txt Access:** {robots_icon}\n")
        f.write(f"- **Googlebot Allowed:** {google_icon}\n\n")
