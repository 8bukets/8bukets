...
def generate_report(data, output_file):
    ...
    # Initialize counters and lists for Single Pass Aggregation
    domain_counter = Counter()
    category_counter = Counter()
    author_counter = Counter()
    dates = []

    # Single pass over data to collect all metrics
    # Optimization: Reduces iteration overhead from O(4N) to O(N)
    for p in data:
        # 1. Domain Analysis
    ...
    # Note: domain_counter contains all domains, so len(domain_counter) gives unique domains
