...
def escape_markdown(text):
    """Escapes Markdown special characters and HTML entities to prevent injection."""
    if not text:
        return ""
    text = str(text)
    # Escape HTML special characters
    text = text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
    # Escape Markdown table pipe to prevent table injection
    text = text.replace("|", "\\|")
    # Escape newlines to prevent table breakage
    text = text.replace("\n", " ").replace("\r", "")
    return text
