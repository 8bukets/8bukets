...
def generate_report(data, output_file):
    ...
    md.append("| :--- | :--- | :--- |")
    md.append(f"| 📝 **Total Posts** | `{total_posts}` | Across {len(year_counts)} years |")
    md.append(f"| 🔗 **Unique Domains** | `{len(set(domains))}` | Linked resources |")
    md.append(f"| 📅 **Date Range** | `{start_date}` to `{end_date}` | Active period |")

    # Top Domains with Bar Chart
    ...
    for year, count in year_counts:
        bar = create_bar_chart(count, max_year_count, length=10)
        md.append(f"| **{year}** | {count} | `{bar}` |")

    # Authors Section (Collapsible if many, but listing all for now as per previous logic)
    md.append("\n## ✍️ Authors")
    ...
    # Footer
    md.append("\n---")
    md.append("\n*Report generated by Markposition Analytics Tool 🎨*")
