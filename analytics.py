...
def create_bar_chart(value, max_value, length=20):
    ...
    if max_value == 0:
        return ""
    num_blocks = int((value / max_value) * length)
    return "█" * num_blocks
...
def generate_report(posts_data, output_file):
    ...
    for domain, count in domain_counts:
        ascii_bar = create_bar_chart(count, max_domain)
    ...
    for cat, count in category_counts:
        ascii_bar = create_bar_chart(count, max_cat)
    ...
    for year, count in year_counts:
        ascii_bar = create_bar_chart(count, max_year)
