"""
Analytics module for processing Markposition data and generating Markdown reports.
"""
...
def create_bar_chart(value, max_value, length=20):
    ...
    if max_value == 0:
        return ""
    # Use math.ceil to ensure at least one block if value > 0 but small,
    # or just simple proportion. Simple proportion is usually fine for ASCII.
...
def generate_report(report_data, output_file):  # pylint: disable=too-many-locals, too-many-statements
    ...
    for domain, count in domain_counts:
        progress_bar = create_bar_chart(count, max_domain_count)
    ...
    for cat, count in category_counts:
        progress_bar = create_bar_chart(count, max_category_count)
    ...
    for year, count in year_counts:
        progress_bar = create_bar_chart(count, max_year_count)
