...
def generate_report(data, output_file):
    ...
    # ⚡ Bolt Optimization: Single Pass Aggregation (O(N))
    # Combined multiple iterations into a single loop to reduce overhead.
    domain_counter = Counter()
    category_counter = Counter()
    author_counter = Counter()
    dates = []

    # We use a set to track unique domains for the general statistics
    unique_domains = set()

    for p in data:
        # 1. Domain Analysis
        # Optimization: Use existing 'domain' field if available to avoid expensive urlparse
        domain = p.get('domain')
        if not domain:
