"""
Analytics module for generating reports from Markposition data.
"""
import json
import argparse
...
def create_bar_chart(value, max_value, length=20):
    ...
    chart = '█' * filled_length + '░' * (length - filled_length)
    return chart
...
def generate_report(posts_data, output_file):
    ...
    md.append("| :--- | :---: | :--- |")
    max_domain = max((c for _, c in domain_counts), default=0)
    for domain, count in domain_counts:
        chart = create_bar_chart(count, max_domain)
    ...
    for cat, count in category_counts:
        chart = create_bar_chart(count, max_cat)
    ...
    for year, count in year_counts:
        chart = create_bar_chart(count, max_year)
