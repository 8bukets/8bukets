...
def generate_report(data, output_file):
    ...
    md.append("- [Top 10 Categories](#categories)")
    md.append("- [Posts by Year](#years)")
    md.append("- [Authors](#authors)")

    md.append("\n<a name='stats'></a>")
    md.append("## 📈 General Statistics")
    md.append(f"- **Total Posts:** {fmt(total_posts)}")
    md.append(f"- **Date Range:** {start_date} to {end_date}")
    md.append(f"- **Unique Domains Linked:** {fmt(len(set(domains)))}")
    md.append("\n[Back to Top](#top)")
    ...
    for year, count in year_counts:
        md.append(f"| {year} | {fmt(count)} |")
    md.append("\n[Back to Top](#top)")

    md.append("\n<a name='authors'></a>")
    md.append("## ✍️ Authors")
    for author, count in author_counts:
        md.append(f"- {author}: {fmt(count)} posts")
    md.append("\n[Back to Top](#top)")
