...
def create_ascii_bar(count, max_count, width=20):
    ...
    # Ensure at least one block if count > 0
    if count > 0 and filled == 0:
        filled = 1
    return '█' * filled

def get_max_count(counts):
    return max((c for _, c in counts), default=0)
...
def generate_report(data, output_file):
    ...
    md.append("| :--- | :---: | :--- |")
    max_domain_count = get_max_count(domain_counts)
    ...
    md.append("| :--- | :---: | :--- |")
    max_cat_count = get_max_count(category_counts)
    ...
    md.append("| :--- | :---: | :--- |")
    max_year_count = get_max_count(year_counts)
