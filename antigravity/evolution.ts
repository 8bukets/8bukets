  ...
  function scan(dir: string) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        if (fullPath.includes('presence.ts') && (!content.includes('resonance_latency_mur') || !content.includes('singularity_readiness_mur'))) {
           suggestions.push({
            file: fullPath.replace(process.cwd(), ''),
            complexity: lines,
            suggestion: 'PHASE_27_MUR_METRICS_MISSING: Presence must include Phase 27 MUR Resonance and Singularity Readiness (Rule 37).'
          })
        }

        if (fullPath.includes('jules.ts') && !content.includes('executePhase27MURPulse()')) {
          suggestions.push({
            file: fullPath.replace(process.cwd(), ''),
            complexity: lines,
            suggestion: 'PHASE_27_MUR_PULSE_MISSING: Jules must execute Phase 27 MUR Pulse (Rule 37).'
          })
        }
      }
    }
