  ...
  async function scan(dir: string) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        if (content.includes('resonance') || content.includes('universal') || content.includes('dni')) {
          const hasPhase27Mandates =
            (content.includes('singularity-readiness') && content.includes('0.99999')) &&
            (content.includes('resonance-latency') && content.includes('<0.01ms')) &&
    ...
    if (s.suggestion.startsWith('PHASE_27_RESONANCE_VIOLATION')) {
      ...
      if (!content.includes('PHASE_27_COMPLIANCE')) {
        const headers = [
          ...
          "/** PHASE 27 COMPLIANCE: DNI-integration-hooks (enabled) **/"
        ].join('\n')
        content = headers + "\n" + content
        await fs.promises.writeFile(fullPath, content)
      }
