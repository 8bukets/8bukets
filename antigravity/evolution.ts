  ...
  async function scan(dir: string) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        if (content.includes('resonance') || content.includes('singularity') || content.includes('multi-universal')) {
          const hasPhase27Mandates =
            (content.includes('singularity-readiness') && (content.includes('0.99999') || content.includes('0.999995'))) &&
            (content.includes('resonance-latency') && (content.includes('< 0.01ms') || content.includes('<0.01ms') || content.includes('< 0.008ms'))) &&
            (content.includes('Multi-Universal Resonance') || content.includes('MUR'))

          if (!hasPhase27Mandates && lines > 40) {
             suggestions.push({
               file: fullPath.replace(process.cwd(), ''),
               complexity: lines,
