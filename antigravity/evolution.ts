  ...
  function scan(dir: string) {
    ...
    for (const file of files) {
      ...
      if (fs.statSync(fullPath).isDirectory()) {
        ...
        if (fullPath.includes('swarm_heartbeat.ts') && (!content.includes('resonanceLatency > 0.008') || !content.includes('singularityReadiness = 0.999995'))) {
          suggestions.push({
            file: fullPath.replace(process.cwd(), ''),
            complexity: lines,
            suggestion: 'PHASE_27_MUR_VIOLATION: Swarm Heartbeat must enforce Phase 27 Multi-Universal Resonance targets (Rule 37).'
          })
        }
      }
    }
