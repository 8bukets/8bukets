        ...
        if (content.includes('resonance') || content.includes('singularity') || content.includes('MUR')) {
          ...
          if (!hasPhase27Mandates && lines > 30) {
            suggestions.push({
              file: fullPath.replace(process.cwd(), ''),
              complexity: lines,
              suggestion: 'PHASE_27_RESONANCE_VIOLATION: Phase 27 mandates singularity-readiness > 0.99999, resonance latency < 0.01ms, and Multi-Universal Resonance (MUR) protocols.'
            })
          }
        }
      }
