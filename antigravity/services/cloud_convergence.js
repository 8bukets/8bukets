...
export class CloudConvergenceService {
    ...
    async synchronizeEcosystem() {
        ...
        try {
            ...
            try {
                const mongoClient = await getMongoClient();
                const db = mongoClient.db();
                await db.collection('system_state').updateOne({ systemId: 'antigravity-alpha-01' }, { $set: { cloud_convergence: state, last_convergence: new Date().toISOString() } }, { upsert: true });
            }
            catch (e) {
    ...
    async resolveConflicts() {
        ...
        try {
            ...
            const isLeader = (_a = presence === null || presence === void 0 ? void 0 : presence.is_leader) !== null && _a !== void 0 ? _a : !isCloud;
            if (isCloud && isLeader) {
            ...
            if (isCloud) {
                ...
                const updatesToPush = localOrders.filter((o) => ['executing', 'completed', 'failed'].includes(o.status));
                if (updatesToPush.length > 0) {
                    ...
                    for (const order of updatesToPush) {
                        const { _id, ...orderData } = order;
                        await db.collection('work_orders').updateOne({ id: order.id }, { $set: { ...orderData, updated_at: new Date().toISOString() } }, { upsert: true });
                    }
                }
