...
export class WorkOrderService {
    ...
    async save(order) {
        ...
        try {
            ...
            if (order) {
                ...
                delete orderData._id; // Ensure we don't try to update the immutable _id
                await db.collection('work_orders').updateOne({ id: order.id }, { $set: orderData }, { upsert: true });
            }
            else {
                // Full sync if no specific order provided
                for (const o of this.orders) {
                    await db.collection('work_orders').updateOne({ id: o.id }, { $set: o }, { upsert: true });
                }
            }
        }
        catch (e) {
    ...
    async createOrder(type, goal, payload, dependsOn) {
    ...
    async updateOrderStatus(id, status, result, error) {
        await this.ensureLoaded();
    ...
    async dispatch(order) {
        ...
        switch (order.type) {
            ...
            case 'SMOKE_TEST': {
                logAutonomousAction(`🧪 [WorkOrder] Running smoke test for ${(_a = order.payload) === null || _a === void 0 ? void 0 : _a.serviceName}...`, 'info');
                ...
                try {
                    ...
                    const testPath = `antigravity/services/${(_b = order.payload) === null || _b === void 0 ? void 0 : _b.serviceName}.test.ts`;
                    if (fs.existsSync(path.join(process.cwd(), testPath))) {
                        await execAsync(`npx vitest run ${testPath}`);
                    }
                    else {
                        logAutonomousAction(`ℹ️ [WorkOrder] No specific test found for ${(_c = order.payload) === null || _c === void 0 ? void 0 : _c.serviceName}. Running general integrity check.`, 'info');
