...
export class GitProviderService {
    ...
    async getActiveProvider() {
        try {
            ...
            if (stdout.includes('@github.com') || stdout.includes('/github.com/'))
                return 'github';
            if (stdout.includes('@gitlab.com') || stdout.includes('/gitlab.com/'))
                return 'gitlab';
        }
        catch (e) { }
        return 'unknown';
    ...
    async push(provider, branch = 'main') {
        ...
        try {
            ...
            if (branch === 'main') {
                try {
                    await this._execAsync('git rebase --abort');
                }
                catch (e) { }
            }
        }
     ...
     */
    async verifyCIStatus(branch, provider = 'github') {
        if (provider === 'github' && process.env.GITHUB_TOKEN) {
            try {
                ...
                if (data.check_runs.length === 0)
                    return true; // No checks is treated as passed
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (statuses.length === 0)
                            return true;
                        return statuses.every((s) => s.status === 'success' || s.status === 'skipped');
                    }
                }
                catch (e) { }
            }
     ...
     */
    async checkApprovals(prId, provider = 'github') {
        var _a, _b, _c;
        ...
        if (provider === 'github' && process.env.GITHUB_TOKEN) {
            ...
            if (projectId) {
                try {
                    ...
                    if (response.ok) {
                        const data = await response.json();
                        const approvalsLeft = (_a = data.approvals_left) !== null && _a !== void 0 ? _a : 0;
                        const approvedByCount = (_c = (_b = data.approved_by) === null || _b === void 0 ? void 0 : _b.length) !== null && _c !== void 0 ? _c : 0;
                        if (approvalsLeft === 0) {
            ...
            try {
                ...
                prs.push(...pulls.map(p => {
                    ...
                    return ({
                        id: p.number,
                        title: p.title,
                        author: ((_a = p.user) === null || _a === void 0 ? void 0 : _a.login) || 'unknown',
                        branch: p.head.ref,
                        status: 'open',
                        provider: 'github'
                    });
     ...
     */
    async mergePullRequest(prId, provider = 'github') {
     ...
     */
    static formatGitKrakenMessage(title, phase, progress, details = []) {
    ...
    static generateProgressBar(percent, length = 20) {
