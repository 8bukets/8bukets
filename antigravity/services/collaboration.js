...
export async function getMissionMetadata() {
    return autonomousFetch(MissionMetadataSchema, async () => {
        ...
        if (!missionStatement)
            missionStatement = 'Autonomous Evolution';
        const stakeholders = [];
...
export async function broadcastToStakeholders(state) {
    var _a, _b, _c, _d, _e;
    const metadata = await getMissionMetadata();
...
`;
    console.log(summary);
    const highIntensitySynergies = ((_b = (_a = state.intelligence.relationshipMap) === null || _a === void 0 ? void 0 : _a.synergies) === null || _b === void 0 ? void 0 : _b.filter((s) => s.intensity === 'High')) || [];
    ...
    const synergySummary = ((_c = state.intelligence.relationshipMap) === null || _c === void 0 ? void 0 : _c.synergies) && state.intelligence.relationshipMap.synergies.length > 0
        ? state.intelligence.relationshipMap.synergies.slice(0, 5).map((s) => `- SYNERGY [${s.intensity}]: ${s.resource} (via ${s.branches.length} branches)`).join('\n')
        : 'No direct resource synergies detected.';
    const recommendations = ((_e = (_d = state.intelligence.relationshipMap) === null || _d === void 0 ? void 0 : _d.collaborationRecommendations) === null || _e === void 0 ? void 0 : _e.length) > 0
        ? state.intelligence.relationshipMap.collaborationRecommendations.slice(0, 10).map((r) => `- [${r.priority}] ${r.action}: ${r.rationale}`).join('\n')
        : 'No immediate collaboration actions required.';
    const detailedBriefing = `--- STRATEGIC SYNERGY ---\n${synergySummary}\n\n--- REQUIRED COORDINATION ---\n${recommendations}`;
    await dispatchExecutiveBriefing(`${synergyAlert} Posture: ${state.docker.status}. Analyzed ${state.intelligence.branches} branches.`, detailedBriefing);
    const logDir = path.join(process.cwd(), 'logs');
    if (!fs.existsSync(logDir))
        fs.mkdirSync(logDir, { recursive: true });
    fs.appendFileSync(path.join(logDir, 'collaboration.log'), summary);
}
export async function mergeEcosystemInsights(branchIntelligence, workOrders) {
    const metadata = await getMissionMetadata();
...
export async function syncCollaborationState(branchIntelligence) {
    ...
    try {
        ...
        const db = client.db();
        await db.collection('system_state').updateOne({ systemId: 'antigravity-alpha-01' }, { $set: newState }, { upsert: true });
        logAutonomousAction('✅ [Collaboration] Autonomous state synchronized to MongoDB.', 'info');
    }
    catch (e) { }
    return newState;
}
export async function mergeBranchInsights(branches) {
