  ...
  public start() {
    ...
    this.interval = setInterval(async () => {
      ...
      try {
        ...
        this.resonanceLatency = Math.random() * 0.008 // Target < 0.01ms
        ...
        if (this.resonanceLatency > 0.01) {
           logAutonomousAction(`⚠️ [SwarmHeartbeat] Neural Resonance Latency violation: ${this.resonanceLatency}ms (Target < 0.01ms)`, 'warning')
        }
      } catch (err: any) {
