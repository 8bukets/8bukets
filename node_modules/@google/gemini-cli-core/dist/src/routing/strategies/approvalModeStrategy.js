...
export class ApprovalModeStrategy {
    ...
    async route(context, config, _baseLlmClient) {
        ...
        if (approvalMode === ApprovalMode.PLAN) {
            const proModel = resolveClassifierModel(model, GEMINI_MODEL_ALIAS_PRO, useGemini3_1, useGemini3_1FlashLite, useCustomToolModel, hasAccessToPreview, config);
            return {
                model: proModel,
                metadata: {
                    source: this.name,
            ...
            const flashModel = resolveClassifierModel(model, GEMINI_MODEL_ALIAS_FLASH, useGemini3_1, useGemini3_1FlashLite, useCustomToolModel, hasAccessToPreview, config);
            return {
                model: flashModel,
                metadata: {
                    source: this.name,
