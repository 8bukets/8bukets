...
export function isSnapshotState(text) {
 ...
 */
export function findLatestSnapshotBaseline(targets) {
    debugLogger.log('[findLatestSnapshotBaseline] Targets:', targets.map((t) => ({
        id: t.id,
        type: t.type,
        text: t.payload &&
                ...
                if (isRecord(parsed)) {
                    ...
                    previousState = {
                        ...
                        discovered_facts: isStringArray(parsed['discovered_facts'])
                            ...
                            : [],
                        constraints_and_preferences: isStringArray(parsed['constraints_and_preferences'])
...
${formatNodesForLlm(nodes)}`;
        ...
        if (isStringArray(resolvedIds)) {
            ...
            newState.active_tasks = newState.active_tasks.filter((t) => !resolvedSet.has(t.id));
        }
        const obsFacts = patch['obsolete_fact_indices'];
        if (isNumberArray(obsFacts)) {
            const dropSet = new Set(obsFacts);
            newState.discovered_facts = newState.discovered_facts.filter((_, i) => !dropSet.has(i));
        }
...
export class SnapshotStateHelper {
     ...
     */
    static flattenAbstracts(nodes, abstractsIds) {
        const pristineIds = [];
