...
export class PipelineOrchestrator {
    ...
    pipelineMutex = new Map();
    pipelineScheduled = new Set();
    nodeProvider;
    ...
    isNodeAllowed(node, triggerTargets, protectedTurnIds = new Set()) {
        return (triggerTargets.has(node.id) &&
            !protectedTurnIds.has(node.id) &&
            !protectedTurnIds.has(node.turnId));
    }
    async executeTriggerSync(trigger, nodes, triggerTargets, protectedTurnIds = new Set()) {
        ...
        for (const pipeline of triggerPipelines) {
            for (const processor of pipeline.processors) {
                try {
                    ...
                    const allowedTargets = currentBuffer.nodes.filter((n) => this.isNodeAllowed(n, triggerTargets, protectedTurnIds));
                    ...
                    const addedNodes = returnedNodes.filter((n) => !allowedTargets.some((at) => at.id === n.id));
                    const removedNodes = allowedTargets.filter((at) => !returnedNodes.some((n) => n.id === at.id));
    ...
    async executeTriggerAsync(trigger, nodes, triggerTargets) {
        const asyncPipelines = this.asyncPipelines.filter((p) => p.triggers.includes(trigger));
        for (const pipeline of asyncPipelines) {
            void this.handleAsyncExecution(pipeline, nodes, triggerTargets);
        }
    }
    async handleAsyncExecution(pipeline, nodes, targets) {
