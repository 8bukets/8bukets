...
export class ContextWorkingBufferImpl {
    ...
    syncPristineHistory(authoritativePristineNodes) {
        ...
        const survivingCurrentNodes = this.nodes
            .filter((n) => {
            if (authoritativeIds.has(n.id))
                return true;
            if (!this.pristineNodesMap.has(n.id))
                return true;
        ...
        const coveredPristineIds = new Set();
        ...
        const pristineIndexMap = new Map(authoritativePristineNodes.map((n, idx) => [n.id, idx]));
