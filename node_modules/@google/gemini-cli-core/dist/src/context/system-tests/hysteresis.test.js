...
describe('Context Manager Hysteresis Tests', () => {
    ...
    const getProjectionTokens = (proj, harness) => proj.reduce((sum, c) => sum + harness.env.tokenCalculator.calculateContentTokens(c.content), 0);
    it('should block consolidation when deficit is below coalescing threshold', async () => {
        ...
        expect(state.finalProjection.some((c) => c.content.parts?.some((p) => p.text?.includes('<SNAPSHOT>')))).toBe(false);
        ...
        expect(state.finalProjection.some((c) => c.content.parts?.some((p) => p.text?.includes('<SNAPSHOT>')))).toBe(true);
    });
