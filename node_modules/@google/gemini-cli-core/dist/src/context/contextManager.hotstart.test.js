...
import { createMockContextConfig, setupContextComponentTest, createMockLlmClient, } from './testing/contextTestUtils.js';
...
describe('ContextManager - Hot Start Calibration', () => {
    ...
    it('should perform calibration exactly once when rendering with existing nodes', async () => {
        ...
        const config = createMockContextConfig(undefined, mockLlm);
        const { contextManager, chatHistory } = setupContextComponentTest(config, stressTestProfile);
        ...
        expect(emitGroundTruthSpy).toHaveBeenCalledWith(expect.objectContaining({
            actualTokens: 42,
            promptBaseUnits: 10,
        }));
...
//# sourceMappingURL=contextManager.hotstart.test.js.map