 ...
 */
export async function render(nodes, orchestrator, sidecar, tracer, env, advancedTokenCalculator, options = {}) {
    ...
    if (!sidecar.config.budget) {
        ...
        const managedNodes = lateBindPrompt && lastTurnId
            ? allVisibleNodes.filter((n) => n.turnId !== lastTurnId)
            : allVisibleNodes;
        const pendingNodes = lateBindPrompt && lastTurnId
            ? allVisibleNodes.filter((n) => n.turnId === lastTurnId)
            : [];
        ...
        const baseUnits = advancedTokenCalculator.getRawBaseUnits(allVisibleNodes) +
            headerBaseUnits;
        return {
            history,
            pendingHistory,
    ...
    if (currentTokens <= maxTokens) {
        ...
        return {
            ...
            pendingHistory,
            didApplyManagement: false,
            baseUnits: advancedTokenCalculator.getRawBaseUnits(allVisibleNodes) +
                headerBaseUnits,
            processedNodes: nodes,
        };
    }
