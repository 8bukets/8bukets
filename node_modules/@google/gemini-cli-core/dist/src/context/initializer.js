...
export async function initializeContextManager(config, chat, lastPromptId) {
    ...
    const getOverheadTokens = () => {
        ...
        return (breakdown.system_instructions +
            breakdown.tool_definitions +
            breakdown.mcp_servers);
    };
    const calculator = new AdaptiveTokenCalculator(charsPerToken, behaviorRegistry, eventBus, getOverheadTokens);
    const env = new ContextEnvironmentImpl(() => config.getBaseLlmClient(), config.getSessionId(), lastPromptId, logDir, projectTempDir, tracer, charsPerToken, eventBus, calculator, behaviorRegistry, {
