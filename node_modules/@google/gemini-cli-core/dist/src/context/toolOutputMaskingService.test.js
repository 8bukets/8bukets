...
describe('ToolOutputMaskingService', () => {
    ...
    it('should never mask exempt tools (like activate_skill) even if they are deep in history', async () => {
        ...
        expect(result.newHistory[1].parts?.[0].functionResponse?.name).toBe('bulky_tool');
        expect((result.newHistory[1].parts?.[0].functionResponse?.response)['output']).toContain(MASKING_INDICATOR_TAG);
    });
});
