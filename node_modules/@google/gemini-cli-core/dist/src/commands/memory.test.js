...
import { applyInboxMemoryPatch, dismissInboxSkill, dismissInboxMemoryPatch, listInboxSkills, listInboxPatches, listInboxMemoryPatches, applyInboxPatch, dismissInboxPatch, listMemoryFiles, moveInboxSkill, refreshMemory, showMemory, } from './memory.js';
...
describe('memory commands', () => {
    ...
    describe('memory patch inbox', () => {
        ...
        let globalMemoryDir;
        let patchConfig;
        const isCaseInsensitivePathPlatform = process.platform === 'win32' || process.platform === 'darwin';
        function buildUpdatePatch(absoluteTargetPath, original, updated) {
        ...
        function buildCreationPatch(absoluteTargetPath, content) {
            const contentLines = content.split('\n');
            const lineCount = content.endsWith('\n')
                ? contentLines.length - 1
                : contentLines.length;
            const additions = (content.endsWith('\n') ? contentLines.slice(0, -1) : contentLines).map((line) => `+${line}`);
            return [
                `--- /dev/null`,
                `+++ ${absoluteTargetPath}`,
                `@@ -0,0 +1,${lineCount} @@`,
        ...
        function swapAsciiPathCase(filePath) {
            return filePath.replace(/[a-z]/gi, (char) => char === char.toLowerCase() ? char.toUpperCase() : char.toLowerCase());
        }
        beforeEach(async () => {
        ...
        it('aggregates all .patch files of a kind into a single inbox entry', async () => {
            ...
            await fs.writeFile(path.join(patchDir, 'a-update.patch'), buildUpdatePatch(target, '- old\n', '- new\n'));
        ...
        it('omits patches whose headers leave the allowed root from the listing', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'escape.patch'), buildCreationPatch(path.join(projectRoot, 'GEMINI.md'), 'Hi.\n'));
            const patches = await listInboxMemoryPatches(patchConfig);
            expect(patches).toHaveLength(0);
            // Direct apply still rejects it (defense-in-depth).
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'escape.patch');
        ...
        it('rejects private patches that target in-root non-memory documents', async () => {
            ...
            for (const [fileName, targetPath] of rejectedTargets) {
                await fs.writeFile(path.join(patchDir, fileName), buildCreationPatch(targetPath, 'rejected\n'));
            }
            const patches = await listInboxMemoryPatches(patchConfig);
            expect(patches).toHaveLength(0);
            for (const [fileName, targetPath] of rejectedTargets) {
                ...
                expect(result.success).toBe(false);
                expect(result.message).toMatch(/outside the private memory root or target allowlist/i);
        ...
        it('omits global patches with disallowed targets from the listing', async () => {
            ...
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'wrong-name.patch'), buildCreationPatch(path.join(globalMemoryDir, 'memory.md'), 'rejected\n'));
        ...
        it('applies a private update patch and removes it from the inbox', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'MEMORY.patch'), buildUpdatePatch(target, '- old\n', '- accepted\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'MEMORY.patch');
        ...
        it.runIf(isCaseInsensitivePathPlatform)('accepts private memory patch targets with different path casing', async () => {
            const target = path.join(memoryTempDir, 'MEMORY.md');
            await fs.writeFile(target, '- old\n');
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'MEMORY.patch'), buildUpdatePatch(swapAsciiPathCase(target), '- old\n', '- accepted\n'));
            const patches = await listInboxMemoryPatches(patchConfig);
            expect(patches).toHaveLength(1);
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'MEMORY.patch');
            expect(result.success).toBe(true);
        ...
        it('applies a private creation patch with a paired MEMORY.md pointer', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            const multiHunkPatch = buildCreationPatch(target, '# Topic\n- new fact\n') +
        ...
        it('auto-bundles a MEMORY.md pointer when the patch creates an orphan sibling', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'orphan-topic.patch'), buildCreationPatch(target, '# Orphan Topic\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'orphan-topic.patch');
            expect(result.success).toBe(true);
            ...
            await expect(fs.access(path.join(patchDir, 'orphan-topic.patch'))).rejects.toThrow();
        });
        it('auto-creates MEMORY.md if it does not exist when bundling pointers', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'fresh-topic.patch'), buildCreationPatch(target, '# Fresh Topic\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'fresh-topic.patch');
            expect(result.success).toBe(true);
            expect(result.message).toMatch(/auto-added MEMORY\.md pointer/i);
            const memoryAfter = await fs.readFile(memoryMd, 'utf-8');
        ...
        it('accepts a private creation patch when MEMORY.md already references the new file', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'later-topic.patch'), buildCreationPatch(target, '# Later Topic\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'later-topic.patch');
            expect(result.success).toBe(true);
        ...
        it('applies a global creation patch to ~/.gemini/GEMINI.md', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'global');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'GEMINI.patch'), buildCreationPatch(target, '# Personal preferences\n- prefer X\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'global', 'GEMINI.patch');
            expect(result.success).toBe(true);
            await expect(fs.readFile(target, 'utf-8')).resolves.toBe('# Personal preferences\n- prefer X\n');
            await expect(fs.access(path.join(patchDir, 'GEMINI.patch'))).rejects.toThrow();
        });
        it('applies a global update patch to ~/.gemini/GEMINI.md', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'global');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'GEMINI.patch'), buildUpdatePatch(target, '- prefer X\n', '- prefer Y\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'global', 'GEMINI.patch');
            expect(result.success).toBe(true);
            await expect(fs.readFile(target, 'utf-8')).resolves.toBe('- prefer Y\n');
            await expect(fs.access(path.join(patchDir, 'GEMINI.patch'))).rejects.toThrow();
        });
        it.runIf(isCaseInsensitivePathPlatform)('accepts global memory patch targets with different path casing', async () => {
            const target = path.join(globalMemoryDir, 'GEMINI.md');
            await fs.writeFile(target, '- prefer X\n');
            const patchDir = path.join(memoryTempDir, '.inbox', 'global');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'GEMINI.patch'), buildUpdatePatch(swapAsciiPathCase(target), '- prefer X\n', '- prefer Y\n'));
            const patches = await listInboxMemoryPatches(patchConfig);
            expect(patches).toHaveLength(1);
            const result = await applyInboxMemoryPatch(patchConfig, 'global', 'GEMINI.patch');
            expect(result.success).toBe(true);
        ...
        it('dismisses a single memory patch from the inbox (legacy single-file mode)', async () => {
            const patchDir = path.join(memoryTempDir, '.inbox', 'global');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'GEMINI.patch'), buildCreationPatch(path.join(globalMemoryDir, 'GEMINI.md'), 'Prefer concise.\n'));
            const result = await dismissInboxMemoryPatch(patchConfig, 'global', 'GEMINI.patch');
            expect(result.success).toBe(true);
            await expect(fs.access(path.join(patchDir, 'GEMINI.patch'))).rejects.toThrow();
        });
        it('apply with relativePath = kind runs every source patch in sequence', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'a-update.patch'), buildUpdatePatch(memoryMd, '- old\n', '- new\n'));
            await fs.writeFile(path.join(patchDir, 'b-topic.patch'), buildUpdatePatch(sibling, 'topic A\n', 'topic B\n'));
        ...
        it('aggregate apply reports successes and failures when one source patch is stale', async () => {
            ...
            await fs.writeFile(path.join(patchDir, 'a-good.patch'), buildUpdatePatch(memoryMd, '- old\n', '- new\n'));
            // Stale patch: context expects something that doesn't exist.
            await fs.writeFile(path.join(patchDir, 'b-stale.patch'), buildUpdatePatch(memoryMd, '- never existed\n', '- attempted\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'private');
            ...
            await expect(fs.access(path.join(patchDir, 'b-stale.patch'))).resolves.toBeUndefined();
        });
        it('dismiss with relativePath = kind removes all source patches', async () => {
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'a.patch'), buildCreationPatch(path.join(memoryTempDir, 'a.md'), 'a\n'));
        ...
        it('rejects global patches that target anything other than ~/.gemini/GEMINI.md', async () => {
            ...
            await fs.writeFile(path.join(patchDir, 'wrong-name.patch'), buildCreationPatch(path.join(globalMemoryDir, 'memory.md'), 'Should be rejected.\n'));
            // Sibling .md files in ~/.gemini/ are also not allowed.
            await fs.writeFile(path.join(patchDir, 'sibling.patch'), buildCreationPatch(path.join(globalMemoryDir, 'notes.md'), 'Should be rejected.\n'));
            // Non-memory files (settings, credentials) must stay off-limits.
            await fs.writeFile(path.join(patchDir, 'settings.patch'), buildCreationPatch(path.join(globalMemoryDir, 'settings.json'), '{"foo": 1}\n'));
            // Child paths under the single allowed file path are not allowed either.
            await fs.writeFile(path.join(patchDir, 'nested.patch'), buildCreationPatch(path.join(globalMemoryDir, 'GEMINI.md', 'nested.md'), 'Should be rejected.\n'));
            ...
            await expect(fs.access(path.join(globalMemoryDir, 'GEMINI.md', 'nested.md'))).rejects.toThrow();
        });
        ...
        it('rejects a creation patch whose target already exists', async () => {
            ...
            const patchDir = path.join(memoryTempDir, '.inbox', 'private');
            await fs.mkdir(patchDir, { recursive: true });
            await fs.writeFile(path.join(patchDir, 'MEMORY.patch'), buildCreationPatch(target, 'replacement\n'));
            const result = await applyInboxMemoryPatch(patchConfig, 'private', 'MEMORY.patch');
            expect(result.success).toBe(false);
            expect(result.message).toMatch(/declares a new file/);
            await expect(fs.readFile(target, 'utf-8')).resolves.toBe('pre-existing\n');
            await expect(fs.access(path.join(patchDir, 'MEMORY.patch'))).resolves.toBeUndefined();
        });
    });
