...
import { getCommandRoots, initializeShellParsers, stripShellWrapper, parseCommandDetails, hasRedirection, detectCommandSubstitution, normalizeCommand, escapeShellArg, } from '../utils/shell-utils.js';
...
function trimLiveOutputBuffer(output) {
    ...
    if (firstCodeUnit >= LOW_SURROGATE_START &&
...
export class ShellToolInvocation extends BaseToolInvocation {
     ...
     */
    wrapCommandForBackgroundPIDs(command, tempFilePath, isWindows) {
        if (isWindows) {
            return command;
        }
    ...
    async execute(options) {
        ...
        if (detectCommandSubstitution(strippedCommand)) {
            return {
                llmContent: 'Command injection detected: command substitution syntax ' +
        ...
        try {
            ...
            const commandToExecute = this.wrapCommandForBackgroundPIDs(strippedCommand, tempFilePath, isWindows);
            ...
            const appendToLiveOutputBuffer = (chunk) => {
                const currentOutput = typeof cumulativeOutput === 'string' ? cumulativeOutput : '';
            ...
            const scheduleTrailingFlush = () => {
                ...
                const trailingDelayMs = Math.max(OUTPUT_UPDATE_INTERVAL_MS - elapsedSinceLastUpdate, 0);
            ...
            return {
                ...
                display: {
                    ...
                    result: typeof returnDisplay === 'string'
...
export class ShellTool extends BaseDeclarativeTool {
    ...
    constructor(context, messageBus) {
        ...
        const definition = getShellDefinition(context.config.isInteractiveShellEnabled(), context.config.getEnableShellOutputEfficiency(), context.config.getSandboxEnabled());
