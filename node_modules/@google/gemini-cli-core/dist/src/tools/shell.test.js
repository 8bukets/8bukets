...
import { ShellTool, OUTPUT_UPDATE_INTERVAL_MS, LIVE_OUTPUT_MAX_BUFFER_CHARS, } from './shell.js';
...
describe('ShellTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should wrap command on linux and parse background PID output', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringMatching(/_bgpids_file=.*gemini-shell-.*[/\\]bgpids\.tmp['"]?\n\(\n {2}trap 'jobs -p > "\$_bgpids_file"' EXIT/), tempRootDir, expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({
                pager: 'cat',
                sanitizationConfig: {},
                sandboxManager: expect.any(Object),
            }));
            expect(wrappedCommand).toMatch(/^_bgpids_file=.*\n\(\n {2}trap 'jobs -p > "\$_bgpids_file"' EXIT\nmy-command &\n\)\n__code=\$\?\nexit \$__code$/);
        ...
        it('should disable PTY execution when interactive shell is unavailable', async () => {
            mockConfig.getEnableInteractiveShell.mockReturnValue(true);
            mockConfig.isInteractiveShellEnabled.mockReturnValue(false);
            const invocation = shellTool.build({ command: 'python --version' });
            const promise = invocation.execute({ abortSignal: mockAbortSignal });
            resolveShellExecution();
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.any(String), tempRootDir, expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({
                pager: 'cat',
            }));
        });
        it('should add a space when command ends with a backslash to prevent escaping newline', async () => {
            ...
            resolveShellExecution();
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringMatching(/_bgpids_file=.*gemini-shell-.*[/\\]bgpids\.tmp/), tempRootDir, expect.any(Function), expect.any(AbortSignal), false, expect.any(Object));
        });
        ...
        it('should use the provided absolute directory as cwd', async () => {
            ...
            resolveShellExecution();
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringMatching(/_bgpids_file=.*gemini-shell-.*[/\\]bgpids\.tmp/), subdir, expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({
                pager: 'cat',
                sanitizationConfig: {},
                sandboxManager: expect.any(Object),
            }));
        ...
        it('should use the provided relative directory as cwd', async () => {
            ...
            resolveShellExecution();
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringMatching(/_bgpids_file=.*gemini-shell-.*[/\\]bgpids\.tmp/), path.join(tempRootDir, 'subdir'), expect.any(Function), expect.any(AbortSignal), false, expect.objectContaining({
                pager: 'cat',
                sanitizationConfig: {},
                sandboxManager: expect.any(Object),
            }));
...
EOF`;
        ...
        it('should format error messages correctly', async () => {
            ...
            expect(result.display).toEqual(expect.objectContaining({
                name: 'Shell',
        ...
        describe('Streaming to `updateOutput`', () => {
            ...
            it('should keep only a bounded text buffer for live display', async () => {
                ...
                expect(updateOutputMock).toHaveBeenCalledOnce();
                expect(updateOutputMock).toHaveBeenLastCalledWith('x'.repeat(LIVE_OUTPUT_MAX_BUFFER_CHARS));
                resolveShellExecution({
            ...
            it('should not start the bounded live text buffer with a low surrogate', async () => {
                ...
                expect(updateOutputMock).toHaveBeenCalledOnce();
                const displayedOutput = updateOutputMock.mock.calls[0][0];
            ...
            it('should not throttle PTY AnsiOutput snapshots in the shell tool', async () => {
                const firstAnsiOutput = [[{ text: 'first' }]];
                const secondAnsiOutput = [[{ text: 'second' }]];
                const invocation = shellTool.build({ command: 'printf output' });
                const promise = invocation.execute({
                    abortSignal: mockAbortSignal,
                    updateOutput: updateOutputMock,
