 ...
 */
function createDynamicOAuthTokenProvider(mcpServerName, mcpServerConfig) {
            ...
            try {
                ...
                if (toolDef.inputSchema) {
                    try {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
                        toolDef.inputSchema = transform(toolDef.inputSchema);
                    }
                    catch (error) {
                        cliConfig.emitMcpDiagnostic('error', `Failed to parse adjusted inputSchema for tool '${toolDef.name}' from server '${mcpServerName}'. Using original schema. Error: ${error instanceof Error ? error.message : String(error)}`, error, mcpServerName);
                    }
                }
...
function createUrlTransport(mcpServerName, mcpServerConfig, transportOptions) {
    ...
    const baseFetch = transportOptions.fetch ??
        globalThis.fetch;
    const httpOptions = {
        ...transportOptions,
...
export async function createTransport(mcpServerName, mcpServerConfig, debugMode, cliConfig) {
    ...
    if (mcpServerConfig.command) {
        ...
        if (debugMode) {
            ...
            const underlyingTransport = transport instanceof McpComplianceTransport
                ? transport.transport
                : transport;
