...
const unwrap = (t) => t instanceof McpComplianceTransport ? t.transport : t;
    ...
    describe('createTransport', () => {
        ...
        describe('should connect via httpUrl', () => {
            it('uses MCP SDK authProvider token() path for oauth-enabled servers', async () => {
                ...
                const testableTransport = unwrap(transport);
            ...
            it('wraps fetch to convert GET 404 to 405 for POST-only servers (e.g. n8n)', async () => {
                ...
                try {
                    const transport = await createTransport('test-server', { httpUrl: 'http://test-server' }, false, MOCK_CONTEXT);
                    const wrappedFetch = unwrap(transport)._fetch;
            ...
            it('respects NO_PROXY for network transports', async () => {
                ...
                try {
                    const transport = await createTransport('test-server', { url: 'http://localhost/sse', type: 'sse' }, false, MOCK_CONTEXT);
...
describe('connectToMcpServer with OAuth', () => {
    ...
    it('should handle automatic OAuth flow on 401 with www-authenticate header', async () => {
        ...
        vi.mocked(mockedClient.connect).mockImplementationOnce(async (transport) => {
            capturedTransport = unwrap(transport);
            return Promise.resolve();
        });
        ...
        const authHeader = unwrap(capturedTransport)
