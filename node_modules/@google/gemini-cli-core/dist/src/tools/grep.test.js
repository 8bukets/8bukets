...
describe('GrepTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should pass -i flag to system grep for case-insensitivity', async () => {
            vi.mocked(execStreaming).mockImplementationOnce(() => createLineGenerator(['fileA.txt:1:hello world']));
            const params = { pattern: 'HELLO' };
            const invocation = grepTool.build(params);
            // Force system grep strategy by mocking isCommandAvailable and ensuring git grep is not used
            invocation.isCommandAvailable = vi.fn(async (command) => {
                if (command === 'git')
                    return false;
                if (command === 'grep')
                    return true;
            ...
            expect(execStreaming).toHaveBeenCalledWith('grep', expect.arrayContaining(['-i']), expect.objectContaining({
