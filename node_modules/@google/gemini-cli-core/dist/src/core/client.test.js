...
\`\`\`
      `.trim();
        ...
        it('should not add context if ideMode is enabled but no open files', async () => {
            ...
            // Assert
            expect(ideContextStore.get).toHaveBeenCalled();
            expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'default-routed-model', isChatModel: true }, initialRequest, expect.any(AbortSignal), expect.objectContaining({ displayContent: undefined }));
        });
      ...
      `.trim();
        ...
        describe('Model Routing', () => {
            ...
            it('should use the model router service to select a model on the first turn', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'routed-model', isChatModel: true }, [{ text: 'Hi' }], expect.any(AbortSignal), expect.objectContaining({ displayContent: undefined }));
            });
            it('should use the same model for subsequent turns in the same prompt (stickiness)', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'routed-model', isChatModel: true }, [{ text: 'Continue' }], expect.any(AbortSignal), expect.objectContaining({ displayContent: undefined }));
            });
            it('should reset the sticky model and re-route when the prompt_id changes', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenCalledWith({ model: 'new-routed-model', isChatModel: true }, [{ text: 'A new topic' }], expect.any(AbortSignal), expect.objectContaining({ displayContent: undefined }));
            });
            it('should re-route within the same prompt when the configured model changes', async () => {
                ...
                await fromAsync(stream);
                expect(mockRouterService.route).toHaveBeenCalledTimes(1);
                expect(mockTurnRunFn).toHaveBeenNthCalledWith(1, { model: 'original-model', isChatModel: true }, [{ text: 'Hi' }], expect.any(AbortSignal), expect.objectContaining({ displayContent: undefined }));
                mockRouterService.route.mockResolvedValue({
                ...
                await fromAsync(stream);
                expect(mockRouterService.route).toHaveBeenCalledTimes(2);
                expect(mockTurnRunFn).toHaveBeenNthCalledWith(2, { model: 'fallback-model', isChatModel: true }, [{ text: 'Continue' }], expect.any(AbortSignal), expect.objectContaining({ displayContent: undefined }));
            });
        });
        ...
        it('should update system instruction when ApprovalModeChanged event is emitted', async () => {
            ...
            vi.mocked(mockConfig.getSystemInstructionMemory).mockReturnValue('Current Memory');
            const { getCoreSystemPrompt } = await import('./prompts.js');
            const mockGetCoreSystemPrompt = vi.mocked(getCoreSystemPrompt);
            mockGetCoreSystemPrompt.mockClear();
            ...
            expect(mockGetCoreSystemPrompt).toHaveBeenCalledWith(mockConfig, 'Current Memory');
        });
        ...
        describe('Availability Service Integration', () => {
            ...
            it('should select first available model, set active, and not consume sticky attempt (done lower in chain)', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenCalledWith(expect.objectContaining({ model: 'model-a' }), expect.anything(), expect.anything(), expect.objectContaining({ displayContent: undefined }));
            });
    ...
    describe('generateContent', () => {
        ...
        describe('Hook System', () => {
            ...
            it('should yield AgentExecutionBlocked and recurse in AfterAgent when hook returns decision: block', async () => {
                ...
                expect(mockTurnRunFn).toHaveBeenNthCalledWith(2, expect.anything(), [{ text: 'Please explain' }], expect.anything(), expect.objectContaining({ displayContent: undefined }));
