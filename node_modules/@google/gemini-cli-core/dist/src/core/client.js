...
export class GeminiClient {
    ...
    async *processTurn(request, signal, prompt_id, boundedTurns, displayContent) {
        ...
        let apiHistoryOverride = undefined;
        if (this.config.getContextManagementConfig().enabled) {
            if (this.contextManager) {
                ...
                const { history: newHistory, apiHistory, pendingApiHistory, baseUnits, } = await this.contextManager.renderHistory(pendingRequest, undefined, signal);
                currentBaseUnits = baseUnits;
                // Use the PROCESSED pending content if available (e.g. if cleaned or distilled)
                const finalPendingContent = pendingApiHistory.length > 0
                    ? pendingApiHistory[0]
                    : rawPendingRequest;
