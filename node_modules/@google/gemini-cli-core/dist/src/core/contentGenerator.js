...
export async function createContentGeneratorConfig(config, authType, apiKey, baseUrl, customHeaders, vertexAiRouting) {
...
export async function createContentGenerator(config, gcConfig, sessionId) {
    const generator = await (async () => {
        if (gcConfig.fakeResponsesNonStrict) {
            const fakeGenerator = await FakeContentGenerator.fromFile(gcConfig.fakeResponsesNonStrict, { nonStrict: true });
        ...
        if (config.authType === AuthType.USE_GEMINI ||
            ...
            const googleGenAI = new GoogleGenAI({
                apiKey: config.authType === AuthType.GATEWAY
                    ? config.apiKey
                    : config.apiKey === ''
                        ? undefined
                        : config.apiKey,
