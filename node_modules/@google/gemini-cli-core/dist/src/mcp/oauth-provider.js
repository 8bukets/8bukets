...
⚠️  Make sure to copy the COMPLETE URL - it may wrap across multiple lines.`);
    ...
    async getValidTokenWithMetadata(serverName, config) {
        const credentials = await this.tokenStorage.getCredentials(serverName);
        if (!credentials)
            return null;
        ...
        if (this.tokenStorage.isTokenExpired(current)) {
            ...
            if (current.refreshToken && clientId && credentials.tokenUrl) {
                try {
                    ...
                    await this.tokenStorage.saveToken(serverName, refreshed, clientId, credentials.tokenUrl, credentials.mcpServerUrl);
                    current = refreshed;
                }
                catch (error) {
                    coreEvents.emitFeedback('error', 'Failed to refresh auth token.', error);
