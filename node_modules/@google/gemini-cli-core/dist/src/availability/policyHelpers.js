...
export function resolvePolicyChain(config, preferredModel, wrapsAround = false) {
    const normalizedPreferredModel = preferredModel
        ...
        : undefined;
    const modelFromConfig = normalizeModelId(normalizedPreferredModel ?? config.getActiveModel?.() ?? config.getModel());
    ...
    const resolvedModel = normalizeModelId(resolveModel(modelFromConfig, useGemini31, useGemini31FlashLite, useCustomToolModel, hasAccessToPreview, config));
    ...
    if (config.getExperimentalDynamicModelConfiguration?.() === true) {
        ...
        if (resolvedModel === DEFAULT_GEMINI_FLASH_LITE_MODEL) {
            ...
            if (!chain) {
                ...
                chain = config.modelConfigService.resolveChain(`${autoPrefix}${chainKey}`, context);
            }
        }
        if (!chain) {
        ...
        if (resolvedModel === DEFAULT_GEMINI_FLASH_LITE_MODEL) {
            ...
            if (hasAccessToPreview) {
                const previewEnabled = isOriginallyGemini3 ||
                    normalizedPreferredModel === PREVIEW_GEMINI_MODEL_AUTO ||
                    configuredModel === PREVIEW_GEMINI_MODEL_AUTO;
...
function applyDynamicSlicing(chain, resolvedModel, wrapsAround) {
    ...
    const activeIndex = chain.findIndex((policy) => normalizeModelId(policy.model) === normalizedResolved);
    if (activeIndex !== -1) {
        return wrapsAround
...
export function buildFallbackPolicyContext(chain, failedModel, wrapsAround = false) {
    ...
    const index = chain.findIndex((policy) => normalizeModelId(policy.model) === normalizedFailed);
    if (index === -1) {
...
export function applyAvailabilityTransition(getContext, failureKind) {
    ...
    if (transition === 'terminal') {
        ...
        context.service.markRetryOncePerTurn(context.policy.model, context.policy.maxAttempts);
