...
describe('Auto Routing Fallback Integration', () => {
    ...
    beforeEach(() => {
        vi.useFakeTimers();
        vi.spyOn(Config.prototype, 'getHasAccessToPreviewModel').mockReturnValue(true);
    ...
    it('should fallback to Flash after 3 tries and try 10 times for Flash in auto mode', async () => {
        ...
        client = new BaseLlmClient(fakeGenerator, config, AuthType.LOGIN_WITH_GOOGLE);
        let attemptsPro = 0;
        let attemptsFlash = 0;
        ...
        vi.spyOn(fakeGenerator, 'generateContent').mockImplementation(async (params) => {
            if (params.model === PREVIEW_GEMINI_MODEL) {
                ...
                attemptsFlash++;
                throw new RetryableQuotaError('Quota exceeded for Flash', mockGoogleApiError, 0);
            }
        ...
        config.setFallbackModelHandler(async (failed, _fallback, _error) => {
    ...
    it('should try 10 times and prompt user in non-auto mode', async () => {
        ...
        const clientNonAuto = new BaseLlmClient(fakeGenerator, configNonAuto, AuthType.LOGIN_WITH_GOOGLE);
        let attemptsPro = 0;
        const mockGoogleApiError = {
            code: 429,
            message: 'Quota exceeded',
        ...
        const handler = vi.fn(async (_failed, _fallback, _error) => 'stop');
        ...
        expect(handler).toHaveBeenCalledWith(PREVIEW_GEMINI_MODEL, PREVIEW_GEMINI_FLASH_MODEL, expect.any(RetryableQuotaError));
    });
    it('should fallback to Flash after 3 tries in experimental dynamic mode', async () => {
        ...
        vi.spyOn(configDynamic, 'getExperimentalDynamicModelConfiguration').mockReturnValue(true);
        const clientDynamic = new BaseLlmClient(fakeGenerator, configDynamic, AuthType.LOGIN_WITH_GOOGLE);
        let attemptsPro = 0;
        let attemptsFlash = 0;
        const mockGoogleApiError = {
            code: 429,
        ...
        configDynamic.setFallbackModelHandler(async (failed, _fallback, _error) => {
            if (failed === PREVIEW_GEMINI_FLASH_MODEL) {
                return 'stop';
            }
            return 'retry_always';
    ...
    it('should retry Pro on next turn after successful fallback to Flash', async () => {
        ...
        config.setFallbackModelHandler(async (_failed, _fallback, _error) => 'retry_always');
...
//# sourceMappingURL=autoRoutingFallback.integration.test.js.map