 ...
 */
describe.skipIf(!isWindows)('ShellExecutionService Windows quoting (real shell)', () => {
    const baseConfig = {
        sanitizationConfig: {
    ...
    async function runReal(command) {
        const controller = new AbortController();
        const handle = await ShellExecutionService.execute(command, process.cwd(), () => { }, controller.signal, false, baseConfig);
    ...
    it('should preserve double quotes inside JSON output', async () => {
        const { result, output } = await runReal(`node -e 'console.log(JSON.stringify({ok:"yes"}))'`);
        expect(result.exitCode).toBe(0);
    ...
    it('should handle a mixed-quote regex literal', async () => {
        const { result, output } = await runReal(`node -e 'console.log(String("a").match(/"/))'`);
        expect(result.exitCode).toBe(0);
...
//# sourceMappingURL=shellExecutionService.windows.integration.test.js.map