...
import { isToolActivityError, } from '../agents/types.js';
...
import { applyParsedSkillPatches, hasParsedPatchHunks, listInboxPatchFiles, validateInboxMemoryPatchFile, } from './memoryPatchUtils.js';
...
function getResolvedActivityFilePath(config, activity) {
    if (!isReadFileActivity(activity)) {
        return null;
    }
    ...
    const targetDir = 'getTargetDir' in config && typeof config.getTargetDir === 'function'
        ? config.getTargetDir()
        : process.cwd();
    ...
    for (const { filePath } of candidates) {
        try {
            ...
            const indexedSession = {
                ...
                memoryScratchpad: conversation.memoryScratchpadIsStale === true
                    ? undefined
...
function formatSessionHeadline(session) {
    ...
    const sanitizedWorkflowSummary = typeof rawWorkflowSummary === 'string'
...
const MEMORY_INBOX_PATCH_KINDS = [
    'private',
    'global',
];
async function validateMemoryInboxPatches(config) {
...
async function buildPendingInboxSummary(memoryDir) {
    ...
    for (const kind of ['private', 'global']) {
        ...
        for (const fileName of patchFiles) {
            ...
            const longestBacktickRun = (content.match(/`+/g) ?? []).reduce((max, run) => Math.max(max, run.length), 2);
    ...
    try {
        ...
        if (lastRun?.runAt) {
            ...
            if (Number.isFinite(lastRunMs) &&
                ...
                debugLogger.log(`[MemoryService] Skipped: last run was ${minutesAgo} minute(s) ago (min interval ${MIN_EXTRACTION_INTERVAL_MS / 60000}m)`);
                return;
            }
        }
