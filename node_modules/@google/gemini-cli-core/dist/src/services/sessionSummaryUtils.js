...
const VALIDATION_COMMAND_REGEX = /\b(test|tests|vitest|jest|pytest|cargo test|npm test|pnpm test|yarn test|bun test|lint|build|check|typecheck)\b/i;
...
async function listSessionFileCandidates(chatsDir) {
    ...
    for (const fileName of allFiles) {
        if (!isSupportedSessionFile(fileName))
            continue;
...
function getValidationStatusForToolCall(toolCall) {
    const command = getToolCallCommand(toolCall);
    const isValidationTool = VALIDATION_TOOL_REGEX.test(toolCall.name) ||
...
async function generateAndSaveSummary(config, sessionPath) {
    ...
    if (!hasCurrentMemoryScratchpad(freshConversation)) {
        metadataUpdate.memoryScratchpad = buildMemoryScratchpad(scratchpadSourceConversation.messages, config.getProjectRoot());
    }
    ...
    try {
        ...
        for (const { filePath, mtimeMs } of sessionFiles) {
            ...
            try {
                ...
                if (!conversation)
                    continue;
                if (conversation.sessionId === config.getSessionId())
                    continue;
                ...
                const userMessageCount = conversation.userMessageCount ??
