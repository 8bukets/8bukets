...
vi.mock('./chatRecordingService.js', async () => {
    const actual = await vi.importActual('./chatRecordingService.js');
    return {
        ...actual,
        loadConversationRecord: vi.fn(actual.loadConversationRecord),
    };
});
function buildLegacySessionJson(fixture) {
...
function buildJsonlSession(fixture) {
    const metadata = {
        sessionId: fixture.sessionId ?? 'session-id',
        projectHash: 'abc123',
        startTime: fixture.startTime ?? '2024-01-01T00:00:00Z',
...
async function writeSession(chatsDir, fileName, contents) {
...
describe('sessionSummaryUtils', () => {
    ...
    describe('getPreviousSession', () => {
        ...
        it('should return null if most recent session already has summary metadata', async () => {
            await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.json', buildLegacySessionJson({
                userMessageCount: 5,
        ...
        it('should return path if most recent session has summary but no scratchpad', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.json', buildLegacySessionJson({
                userMessageCount: 5,
                summary: 'Existing summary',
            }));
            const result = await getPreviousSession(mockConfig);
        ...
        it('should return null if most recent session has 1 or fewer user messages', async () => {
            await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.json', buildLegacySessionJson({ userMessageCount: 1 }));
            const result = await getPreviousSession(mockConfig);
            expect(result).toBeNull();
        });
        it('should return path if most recent session has more than 1 user message and no summary', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.json', buildLegacySessionJson({ userMessageCount: 2 }));
            const result = await getPreviousSession(mockConfig);
            expect(result).toBe(filePath);
        });
        it('should select most recently updated session', async () => {
            await writeSession(chatsDir, 'session-2024-01-01T10-00-older000.json', buildLegacySessionJson({
                userMessageCount: 2,
                lastUpdated: '2024-01-01T10:00:00Z',
            }));
            const newerPath = await writeSession(chatsDir, 'session-2024-01-02T10-00-newer000.json', buildLegacySessionJson({
                userMessageCount: 2,
        ...
        it('should ignore corrupted session files', async () => {
            await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.json', 'invalid json');
            const result = await getPreviousSession(mockConfig);
            expect(result).toBeNull();
        });
        it('should support JSONL sessions and sort by lastUpdated instead of filename', async () => {
            await writeSession(chatsDir, 'session-2024-01-02T10-00-older000.jsonl', buildJsonlSession({
                userMessageCount: 2,
                lastUpdated: '2024-01-01T10:00:00Z',
                sessionId: 'older-session',
            }));
            const newerPath = await writeSession(chatsDir, 'session-2024-01-01T10-00-newer000.jsonl', buildJsonlSession({
                userMessageCount: 2,
        ...
        it('should stop scanning once older mtimes cannot beat the best lastUpdated', async () => {
            const loadConversationRecord = vi.mocked(chatRecordingService.loadConversationRecord);
            const currentPath = await writeSession(chatsDir, 'session-2024-01-03T10-00-cur00001.jsonl', buildJsonlSession({
                sessionId: 'current-session',
                userMessageCount: 2,
                lastUpdated: '2024-01-03T10:00:00Z',
            }));
            await setSessionMtime(currentPath, '2024-01-03T10:00:00Z');
            const bestPath = await writeSession(chatsDir, 'session-2024-01-02T10-00-best0001.jsonl', buildJsonlSession({
                sessionId: 'best-session',
                userMessageCount: 2,
                lastUpdated: '2024-01-02T10:00:00Z',
            }));
            await setSessionMtime(bestPath, '2024-01-02T10:00:00Z');
            const olderPath = await writeSession(chatsDir, 'session-2024-01-01T10-00-older001.jsonl', buildJsonlSession({
                sessionId: 'older-session',
                userMessageCount: 2,
                lastUpdated: '2024-01-01T10:00:00Z',
            }));
        ...
        it('should skip subagent sessions when backfilling scratchpads', async () => {
            const mainPath = await writeSession(chatsDir, 'session-2024-01-01T10-00-main0001.jsonl', buildJsonlSession({
            ...
            await writeSession(chatsDir, 'session-2024-01-02T10-00-sub00001.jsonl', buildJsonlSession({
    ...
    describe('generateSummary', () => {
        ...
        it('should generate and save summary for legacy JSON sessions', async () => {
            ...
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.json', buildLegacySessionJson({ userMessageCount: 2, lastUpdated }));
        ...
        it('should handle errors gracefully without throwing', async () => {
            await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.json', buildLegacySessionJson({ userMessageCount: 2 }));
        ...
        it('should append a metadata update when saving a summary to JSONL', async () => {
            const lastUpdated = '2024-01-01T10:00:00Z';
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-abc12345.jsonl', buildJsonlSession({ userMessageCount: 2, lastUpdated }));
            await generateSummary(mockConfig);
            expect(mockGenerateSummary).toHaveBeenCalledTimes(1);
        ...
        it('should backfill scratchpad without regenerating summary', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-backfill.jsonl', buildJsonlSession({
                userMessageCount: 2,
                summary: 'Existing summary',
            }));
            await generateSummary(mockConfig);
        ...
        it('should not retry summary generation after writing a scratchpad fallback', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-summary-fallback.jsonl', buildJsonlSession({
            ...
            await generateSummary(mockConfig);
            expect(mockGenerateSummary).toHaveBeenCalledTimes(1);
            const savedConversation = await chatRecordingService.loadConversationRecord(filePath);
        ...
        it('should refresh stale scratchpads when messages were appended after metadata', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-resumed1.jsonl', buildJsonlSession({
        ...
        it('should preserve a newer JSONL lastUpdated written concurrently', async () => {
            ...
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-race.jsonl', buildJsonlSession({
                userMessageCount: 2,
                lastUpdated: initialLastUpdated,
            }));
            const actualChatRecordingService = await vi.importActual('./chatRecordingService.js');
            let injectedConcurrentUpdate = false;
            let sessionReadCount = 0;
            vi.mocked(chatRecordingService.loadConversationRecord).mockImplementation(async (targetPath, options) => {
                const conversation = await actualChatRecordingService.loadConversationRecord(targetPath, options);
                ...
                if (!injectedConcurrentUpdate &&
                    ...
                    await fs.appendFile(filePath, `${JSON.stringify({ $set: { lastUpdated: newerLastUpdated } })}\n`);
                }
                return conversation;
            });
            await generateSummary(mockConfig);
        ...
        it('should skip the active startup session and summarize the previous session', async () => {
            const previousPath = await writeSession(chatsDir, 'session-2024-01-01T10-00-prev0001.jsonl', buildJsonlSession({
                sessionId: 'previous-session',
                userMessageCount: 2,
                lastUpdated: '2024-01-01T10:00:00Z',
            }));
            const currentPath = await writeSession(chatsDir, 'session-2024-01-02T10-00-cur00001.jsonl', buildJsonlSession({
                sessionId: 'current-session',
                userMessageCount: 1,
                lastUpdated: '2024-01-02T10:00:00Z',
            }));
        ...
        it('should preserve repo-root file names in scratchpad touched paths', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-rootpath.jsonl', buildJsonlSession({
        ...
        it('should summarize shell commands without raw arguments in scratchpad tool sequence', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-shellcmd.jsonl', buildJsonlSession({
                        ...
                        toolCalls: [
                            {
                                ...
                                args: {
                                    command: 'curl https://api.example.com -H "Authorization: Bearer sk-secret-token"',
                                },
                                status: CoreToolCallStatus.Success,
                                timestamp: '2024-01-01T00:00:01Z',
                            },
                            {
                                ...
                                name: 'run_shell_command',
                                args: {
                                    command: 'DATABASE_URL=postgresql://user:password@localhost/db npm run migrate -- --name add-users',
                                },
                                status: CoreToolCallStatus.Success,
                                timestamp: '2024-01-01T00:00:02Z',
                            },
            ...
            expect(savedConversation?.memoryScratchpad?.workflowSummary).not.toContain('Authorization');
            expect(savedConversation?.memoryScratchpad?.workflowSummary).not.toContain('sk-secret-token');
            expect(savedConversation?.memoryScratchpad?.workflowSummary).not.toContain('password');
            expect(savedConversation?.memoryScratchpad?.workflowSummary).not.toContain('add-users');
        });
        it('should not classify validation substrings as validation tools', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-validation-substring.jsonl', buildJsonlSession({
        ...
        it('should cap nested path extraction depth', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-deep-paths.jsonl', buildJsonlSession({
        ...
        it('should use the latest validation result in scratchpad metadata', async () => {
            const filePath = await writeSession(chatsDir, 'session-2024-01-01T10-00-validation.jsonl', buildJsonlSession({
