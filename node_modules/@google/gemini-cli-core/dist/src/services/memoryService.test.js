...
describe('memoryService', () => {
    ...
    describe('startMemoryService', () => {
        ...
        it('records inbox patches as memoryCandidatesCreated without applying them', async () => {
            ...
            vi.mocked(LocalAgentExecutor.create).mockResolvedValueOnce({
                run: vi.fn().mockImplementation(async () => {
                    ...
                    await fs.writeFile(path.join(inboxDir, 'private', 'MEMORY.patch'), [
                        `--- /dev/null`,
                    ...
                    await fs.writeFile(path.join(inboxDir, 'global', 'reply-style.patch'), [
                        `--- /dev/null`,
            ...
            for (const relativePath of [
                ...
                await expect(fs.access(path.join(memoryDir, relativePath))).resolves.toBeUndefined();
            }
            ...
            expect(state.runs.at(-1)?.memoryCandidatesCreated ?? []).toEqual(expect.arrayContaining([
                path.join('.inbox', 'private', 'MEMORY.patch'),
                path.join('.inbox', 'global', 'reply-style.patch'),
            ]));
        });
        it('drops malformed memory inbox patches before recording or notifying', async () => {
            ...
            vi.mocked(LocalAgentExecutor.create).mockResolvedValueOnce({
                run: vi.fn().mockImplementation(async () => {
                    ...
                    await fs.writeFile(malformedPatchPath, [
                        `--- /dev/null`,
                        `+++ ${path.join(memoryDir, 'MEMORY.md')}`,
                        `@@ -0,0 +1,1 @@`,
        ...
        it('records only sessions whose read_file completed successfully as processed', async () => {
            ...
            const failedPath = path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-03T00-00-failed.jsonl`);
            const rejectedPath = path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-02T00-00-rejected.jsonl`);
            const mismatchedEndPath = path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-02T00-00-mismatched-end.jsonl`);
            const mismatchedErrorPath = path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-02T00-00-mismatched-error.jsonl`);
            ...
            await writeConversationJsonl(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-skipped.jsonl`), skippedConversation);
    ...
    describe('buildSessionIndex', () => {
        ...
        it('falls back to scratchpad workflow summary when summary is missing', async () => {
            ...
            const conversation = createConversation({
                ...
                memoryScratchpad: {
                    version: 1,
                    workflowSummary: 'read_file -> edit | paths packages/core/src/services/memoryService.ts | validated',
                },
                messageCount: 20,
            });
            await writeConversationJsonl(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-scratch01.jsonl`), conversation);
            const result = await buildSessionIndex(chatsDir, { runs: [] });
        ...
        it('ignores malformed scratchpad workflow summaries while indexing sessions', async () => {
            ...
            await writeConversationJsonl(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-valid.jsonl`), validConversation);
            const result = await buildSessionIndex(chatsDir, { runs: [] });
        ...
        it('appends workflow summary when both summary and scratchpad are present', async () => {
            ...
            const conversation = createConversation({
                ...
                memoryScratchpad: {
                    version: 1,
                    workflowSummary: 'read_file -> edit | paths packages/core/src/services/sessionSummaryUtils.ts',
                },
                messageCount: 20,
            });
            await writeConversationJsonl(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-scratch02.jsonl`), conversation);
            const result = await buildSessionIndex(chatsDir, { runs: [] });
        ...
        it('omits stale scratchpad workflow summaries from resumed JSONL sessions', async () => {
            ...
            const filePath = path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-stale001.jsonl`);
        ...
        it('sanitizes shell command workflow summaries before indexing sessions', async () => {
            ...
            const conversation = createConversation({
                ...
                memoryScratchpad: {
                    version: 1,
                    workflowSummary: 'run_shell_command: curl https://api.example.com -H "Authorization: Bearer sk-secret-token" -> read_file | paths package.json',
                },
                messageCount: 20,
            });
            await writeConversationJsonl(path.join(chatsDir, `${SESSION_FILE_PREFIX}2025-01-01T00-00-shellraw.jsonl`), conversation);
            const result = await buildSessionIndex(chatsDir, { runs: [] });
            expect(result.sessionIndex).toContain('workflow: run_shell_command: curl -> read_file | paths package.json');
