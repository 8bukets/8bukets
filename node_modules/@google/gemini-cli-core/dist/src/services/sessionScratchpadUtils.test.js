...
import { sanitizeWorkflowSummaryForScratchpad, summarizeShellCommandForScratchpad, } from './sessionScratchpadUtils.js';
describe('sessionScratchpadUtils', () => {
    describe('summarizeShellCommandForScratchpad', () => {
        it('summarizes quoted and assignment-prefixed shell commands', () => {
            ...
            expect(summarizeShellCommandForScratchpad('DATABASE_URL=postgres://user:password@example/db pnpm test')).toBe('pnpm');
        });
    ...
    describe('sanitizeWorkflowSummaryForScratchpad', () => {
        it('sanitizes adversarial shell commands in workflow summaries', () => {
            const adversarialCommand = `"${'\\"!'.repeat(10_000)}`;
            expect(sanitizeWorkflowSummaryForScratchpad(`${SHELL_TOOL_NAME}: ${adversarialCommand} -> read_file`)).toBe(`${SHELL_TOOL_NAME}: shell -> read_file`);
        });
    });
});
//# sourceMappingURL=sessionScratchpadUtils.test.js.map