...
describe('Flash Model Fallback Configuration', () => {
    ...
    describe('activateFallbackMode', () => {
        ...
        it('should set fallback override when failedModel is provided and register runtime override', () => {
            ...
            expect(config.getModel()).toBe(DEFAULT_GEMINI_FLASH_MODEL);
            expect(config.getFallbackOverride(DEFAULT_GEMINI_MODEL)).toBe(DEFAULT_GEMINI_FLASH_MODEL);
            // Verify it registers the runtime model override with ModelConfigService
            expect(config
                .getModelConfigService()
                .getResolvedConfig({ model: DEFAULT_GEMINI_MODEL }).model).toBe(DEFAULT_GEMINI_FLASH_MODEL);
        });
        it('should flatten override chains when a model that was previously a target fails', () => {
            ...
            expect(config.getModelConfigService().getResolvedConfig({ model: 'model-A' })
                .model).toBe('model-B');
            ...
            expect(config.getModelConfigService().getResolvedConfig({ model: 'model-A' })
                .model).toBe('model-C');
            expect(config.getModelConfigService().getResolvedConfig({ model: 'model-B' })
                .model).toBe('model-C');
        });
