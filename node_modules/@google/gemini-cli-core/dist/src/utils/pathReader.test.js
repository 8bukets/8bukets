...
describe('readPathFromWorkspace', () => {
    ...
    it('should prevent path traversal outside the workspace via relative paths', async () => {
        ...
        await expect(readPathFromWorkspace(relativeTraversal, config)).rejects.toThrow(`Path not found in workspace: ${relativeTraversal}`);
    });
    it('should prevent symlink escape outside the workspace', async () => {
        ...
        await expect(readPathFromWorkspace('malicious-link', config)).rejects.toThrow('Path not found in workspace: malicious-link');
    });
    it('should block symlink escape inside a directory expansion (defense-in-depth)', async () => {
        ...
        expect(resultText).toContain('--- Skipped malicious-link.txt: traverses outside workspace ---');
    });
    it('should push multiple skip messages if multiple traversals are found in a directory', async () => {
        ...
        expect(resultText).toContain('--- Skipped link1.txt: traverses outside workspace ---');
        expect(resultText).toContain('--- Skipped link2.txt: traverses outside workspace ---');
