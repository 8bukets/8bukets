 ...
 */
export async function resolveAtCommandPath(pathName, config, onDebugMessage = () => { }) {
    ...
    if (!pathValidation.isValid) {
        ...
        if (extractedPath && extractedPath !== pathName) {
            onDebugMessage(`Identified invalid path fragment, attempting to extract path: "${extractedPath}" from "${pathName}"`);
            // Recurse once with the extracted path.
            return resolveAtCommandPath(extractedPath, config, onDebugMessage);
        }
        onDebugMessage(`Skipping invalid path in @-command: ${pathName}. Reason: ${pathValidation.error}`);
    ...
    if (path.isAbsolute(pathName)) {
        ...
        if (validationError) {
            onDebugMessage(`Skipping unauthorized absolute path: ${pathName}. Reason: ${validationError}`);
            return {
                status: 'unauthorized',
        ...
        try {
            ...
            onDebugMessage(`Unexpected error stating path ${pathName}: ${getErrorMessage(error)}`);
            return { status: 'not_found' };
        }
    }
    ...
    for (const dir of workspaceDirs) {
        ...
        if (validationError) {
            onDebugMessage(`Skipping unauthorized path: ${absolutePath}. Reason: ${validationError}`);
        ...
        try {
            ...
            onDebugMessage(`Unexpected error stating path ${absolutePath}: ${getErrorMessage(error)}`);
        }
    }
    if (lastUnauthorized) {
 ...
 */
function tryExtractPath(noisyString) {
...
//# sourceMappingURL=atCommandUtils.js.map