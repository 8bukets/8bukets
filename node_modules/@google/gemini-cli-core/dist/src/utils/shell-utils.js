 ...
 */
export function stripHupGuard(command) {
...
export function resolveExecutable(exe) {
...
export function detectCommandSubstitution(command) {
    ...
    const isPowerShell = typeof shell === 'string' &&
...
const POWERSHELL_KEYWORD_RE = /\b(if|elseif|else|foreach|for|while|do|switch|try|catch|finally|until|trap|function|filter)(\s+[-\w]+)*\s*$/i;
