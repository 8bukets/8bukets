...
import { getAllGeminiMdFilenames, PROJECT_MEMORY_INDEX_FILENAME, } from '../tools/memoryTool.js';
import { processImports } from './memoryImportProcessor.js';
import { GEMINI_DIR, homedir, isSubpath, normalizePath, toAbsolutePath, } from './paths.js';
    ...
    for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
        ...
        const batchPromises = batch.map(async (filePath) => {
            try {
                ...
                const isEISDIR = error instanceof Error &&
                    error.code === 'EISDIR';
                if (isEISDIR) {
...
export async function getGlobalMemoryPaths() {
    ...
    const accessChecks = geminiMdFilenames.map(async (filename) => {
        const globalPath = toAbsolutePath(path.join(userHome, GEMINI_DIR, filename));
        try {
...
export async function getUserProjectMemoryPaths(projectMemoryDir) {
    const preferredMemoryPath = toAbsolutePath(path.join(projectMemoryDir, PROJECT_MEMORY_INDEX_FILENAME));
    try {
