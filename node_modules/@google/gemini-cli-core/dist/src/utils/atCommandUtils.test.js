...
describe('atCommandUtils', () => {
    ...
    it('should resolve a valid path', async () => {
        ...
        const result = await resolveAtCommandPath('file.ts', mockConfig);
    ...
    it('should resolve an absolute path', async () => {
        ...
        const result = await resolveAtCommandPath(absolutePath, mockConfig);
        expect(result.status).toBe('resolved');
        if (result.status === 'resolved') {
    ...
    it('should handle multiple directories in workspace context', async () => {
        mockWorkspaceContext['getDirectories'].mockReturnValue([
            '/dir1',
            '/dir2',
        ]);
        const mockStats = {
        ...
        const result = await resolveAtCommandPath('file.txt', mockConfig);
        expect(result.status).toBe('resolved');
        if (result.status === 'resolved') {
    ...
    it('should return invalid for invalid path (too long)', async () => {
        ...
        const result = await resolveAtCommandPath(longPath, mockConfig);
        expect(result.status).toBe('invalid');
    });
    it('should return invalid for path with log markers (and no valid subpath)', async () => {
        const onDebug = vi.fn();
        const result = await resolveAtCommandPath('FAIL AssertionError: expected true to be false', mockConfig, onDebug);
        expect(result.status).toBe('invalid');
        expect(onDebug).toHaveBeenCalledWith(expect.stringContaining('Skipping invalid path'));
    });
    it('should return not_found if path does not exist in any workspace directory', async () => {
        ...
        const result = await resolveAtCommandPath('nonexistent.ts', mockConfig);
    ...
    it('should resolve directory paths correctly', async () => {
        ...
        vi.mocked(fsPromises.stat).mockResolvedValue(mockStats);
        const result = await resolveAtCommandPath('src', mockConfig);
        expect(result.status).toBe('resolved');
        if (result.status === 'resolved') {
    ...
    it('should respect validatePathAccess for paths within root', async () => {
        mockConfig['validatePathAccess'].mockReturnValue('Unauthorized access');
        // Mock getTargetDir to match the resolved path so it's considered "within root"
        mockConfig['getTargetDir'].mockReturnValue('/mock/root');
        const result = await resolveAtCommandPath('secret.txt', mockConfig);
        expect(result.status).toBe('unauthorized');
    });
    it('should return unauthorized for paths outside root', async () => {
        mockConfig['validatePathAccess'].mockReturnValue('Outside workspace');
        mockConfig['getTargetDir'].mockReturnValue('/mock/workspace');
        const mockStats = {
            isDirectory: () => false,
            isFile: () => true,
        };
        ...
        const result = await resolveAtCommandPath('outside.txt', mockConfig);
        expect(result.status).toBe('unauthorized');
    ...
    it('should not treat paths with shared prefixes as subpaths if not actually inside', async () => {
        ...
        mockWorkspaceContext['getDirectories'].mockReturnValue([dir]);
        const mockStats = {
            isDirectory: () => false,
            isFile: () => true,
        };
        vi.mocked(fsPromises.stat).mockResolvedValue(mockStats);
        const result = await resolveAtCommandPath(otherPath, mockConfig);
        expect(result.status).toBe('resolved');
        if (result.status === 'resolved') {
    ...
    it('should resolve paths in deeply nested workspace directories', async () => {
        ...
        vi.mocked(fsPromises.stat).mockResolvedValue(mockStats);
        const result = await resolveAtCommandPath(absFile, mockConfig);
        expect(result.status).toBe('resolved');
        if (result.status === 'resolved') {
    ...
    it('should extract and resolve a buried path from a log fragment', async () => {
        ...
        const result = await resolveAtCommandPath(logFragment, mockConfig);
        expect(result.status).toBe('resolved');
        if (result.status === 'resolved') {
    ...
    describe('Best-Effort Path Extraction (tryExtractPath)', () => {
        ...
        it('should extract path from "AssertionError: ..." format', async () => {
            const result = await resolveAtCommandPath(`AssertionError: expected something but got something else at ${mockFile}:10:5`, mockConfig);
            expect(result.status).toBe('resolved');
            if (result.status === 'resolved') {
        ...
        it('should extract path wrapped in parentheses', async () => {
            const result = await resolveAtCommandPath(`FAIL (${mockFile})`, mockConfig);
            expect(result.status).toBe('resolved');
            if (result.status === 'resolved') {
                expect(result.resolved.absolutePath).toBe(absMockFile);
            }
        ...
        it('should extract path from "✓" pass marker', async () => {
            const result = await resolveAtCommandPath(`✓ ${mockFile}`, mockConfig);
            expect(result.status).toBe('resolved');
        });
        ...
        it('should NOT strip traversal (..), but let central validation handle it', async () => {
            ...
            mockConfig['validatePathAccess'].mockImplementation((p) => {
                if (p === absPath)
                    return 'Outside workspace';
                return null;
            });
            const result = await resolveAtCommandPath(`FAIL ${traversalPath}`, mockConfig);
        ...
        it('should reject paths with null bytes via validatePath', async () => {
            ...
            const result = await resolveAtCommandPath(`FAIL ${nullBytePath}`, mockConfig);
        ...
        it('should handle paths with slashes and extensions correctly', async () => {
            ...
            const result = await resolveAtCommandPath(`FAIL ${complexPath}:123`, mockConfig);
            expect(result.status).toBe('resolved');
            if (result.status === 'resolved') {
        ...
        it('should fail gracefully if no valid path can be extracted', async () => {
            const result = await resolveAtCommandPath('FAIL some random text with no slashes or dots', mockConfig);
            expect(result.status).toBe('invalid');
        });
        it('should return unauthorized if the extracted path is not authorized', async () => {
            const secretFile = '/etc/passwd';
            mockConfig['validatePathAccess'].mockImplementation((p) => p === secretFile ? 'Unauthorized' : null);
            vi.mocked(fsPromises.stat).mockResolvedValue(mockStats);
            const result = await resolveAtCommandPath(`FAIL ${secretFile}`, mockConfig);
    ...
    it('should include reason in debug message for unauthorized paths', async () => {
        const onDebug = vi.fn();
        mockConfig['validatePathAccess'].mockReturnValue('FORBIDDEN_ZONE');
        await resolveAtCommandPath('secret.txt', mockConfig, onDebug);
        expect(onDebug).toHaveBeenCalledWith(expect.stringContaining('Reason: FORBIDDEN_ZONE'));
    });
});
//# sourceMappingURL=atCommandUtils.test.js.map