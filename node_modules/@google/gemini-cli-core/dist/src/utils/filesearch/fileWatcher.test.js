...
const waitForEvent = async (events, predicate, timeoutMs = 4000) => {
...
describe('FileWatcher', () => {
    ...
    it('should emit addDir and unlinkDir events for directories', async () => {
        ...
        await waitForEvent(events, (event) => event.eventType === 'addDir' && event.relativePath === dirName);
        await fs.rm(dirPath, { recursive: true, force: true });
        await waitForEvent(events, (event) => event.eventType === 'unlinkDir' && event.relativePath === dirName);
        await watcher.close();
    });
    it('should normalize nested paths without leading dot prefix', async () => {
        ...
        await waitForEvent(events, (event) => event.eventType === 'add' && event.relativePath === 'nested/file.txt');
        const nestedFileEvent = events.find((event) => event.eventType === 'add' && event.relativePath.endsWith('/file.txt'));
    ...
    it('should not emit new events after stop is called', async () => {
        ...
        await waitForEvent(events, (event) => event.eventType === 'add' && event.relativePath === 'before-stop.txt');
        await watcher.close();
    ...
    it('should be safe to start and stop multiple times', async () => {
        ...
        await waitForEvent(events, (event) => event.eventType === 'add' && event.relativePath === 'idempotent.txt');
        await watcher.close();
        await watcher.close();
