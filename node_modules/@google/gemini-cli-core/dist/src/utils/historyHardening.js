...
import {} from '../core/agentChatHistory.js';
 ...
 */
export function hardenHistory(history, options = {}) {
    if (history.length === 0)
        return history;
        ...
        if (turn.content.role === 'model') {
            ...
            if (callParts.length > 0) {
                ...
                for (const call of callParts) {
                    ...
                    const hasResponse = nextTurn?.content.role === 'user' &&
                        nextTurn.content.parts?.some((p) => p.functionResponse?.id === id &&
                            p.functionResponse?.name === name);
                    if (!hasResponse) {
                        debugLogger.log(`[HistoryHardener] Call id='${id}' (name='${name}') has no matching response in next turn.`);
            ...
            for (const p of parts) {
                if (p.functionResponse) {
                    ...
                    const hasCall = prevTurn?.content.role === 'model' &&
                        prevTurn.content.parts?.some((cp) => cp.functionCall?.id === id && cp.functionCall?.name === name);
                    if (hasCall) {
                        validParts.push(p);
                    }
                    else {
                        debugLogger.log(`[HistoryHardener] Orphaned functionResponse id='${id}' (name='${name}'). Injecting synthetic functionCall.`);
            ...
            if (orphanedResponses.length > 0) {
                ...
                for (const orph of orphanedResponses) {
                    ...
                    const hasExistingCall = targetModelTurn.content.parts.some((p) => !!p.functionCall);
                    const callPart = {
                        functionCall: {
...
function refineToolResponses(history) {
    for (let i = 1; i < history.length; i++) {
        ...
        if (turn.content.role === 'user' && prev.content.role === 'model') {
            const callOrder = prev.content.parts
                ?.filter((p) => !!p.functionCall)
                .map((p) => p.functionCall.id) || [];
            if (callOrder.length > 0) {
                const responseParts = turn.content.parts?.filter((p) => !!p.functionResponse) || [];
 ...
 */
export function scrubHistory(history) {
 ...
 */
export function scrubContents(contents) {
...
export function scrubPart(part) {
    const scrubbed = {};
...
//# sourceMappingURL=historyHardening.js.map