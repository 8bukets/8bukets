...
describe('PolicyEngine - Core Tools Mapping', () => {
    ...
    it('should allow tools explicitly listed in settings.tools.core', async () => {
        ...
        const config = await createPolicyEngineConfig(settings, ApprovalMode.DEFAULT, undefined, true);
        const engine = new PolicyEngine(config);
        // Test simple tool name
        const result1 = await engine.check({ name: 'run_shell_command', args: { command: 'ls' } }, undefined);
        expect(result1.decision).toBe(PolicyDecision.ALLOW);
        // Test tool name with args
        const result2 = await engine.check({ name: 'run_shell_command', args: { command: 'git status' } }, undefined);
        expect(result2.decision).toBe(PolicyDecision.ALLOW);
        // Test tool not in core list
        const result3 = await engine.check({ name: 'run_shell_command', args: { command: 'npm test' } }, undefined);
    ...
    it('should allow tools in tools.core even if they are restricted by default policies', async () => {
        ...
        const config = await createPolicyEngineConfig(settings, ApprovalMode.DEFAULT, undefined, true);
        const engine = new PolicyEngine(config);
        const result = await engine.check({ name: 'run_shell_command', args: { command: 'any command' } }, undefined);
        expect(result.decision).toBe(PolicyDecision.ALLOW);
    });
});
//# sourceMappingURL=core-tools-mapping.test.js.map