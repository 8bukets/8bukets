...
describe('PolicyEngine Command Substitution Validation', () => {
    ...
    const setupEngine = (blockedCmd) => new PolicyEngine({
    ...
    it('should block echo $(dangerous_cmd) when dangerous_cmd is explicitly blocked', async () => {
        ...
        const result = await engine.check({ name: 'run_shell_command', args: { command: 'echo $(dangerous_cmd)' } }, 'test-server');
    ...
    it('should block commands inside subshells (dangerous_cmd)', async () => {
        const engine = setupEngine('dangerous_cmd');
        const result = await engine.check({ name: 'run_shell_command', args: { command: '(dangerous_cmd)' } }, 'test-server');
        expect(result.decision).toBe(PolicyDecision.DENY);
    });
...
//# sourceMappingURL=shell-substitution.test.js.map