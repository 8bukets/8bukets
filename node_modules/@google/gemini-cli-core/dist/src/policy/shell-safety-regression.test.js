...
describe('PolicyEngine - Shell Safety Regression Suite', () => {
    ...
    const setupEngine = (allowedCommands) => {
    ...
    it('should block unauthorized chained command with &&', async () => {
        ...
        const result = await engine.check({ name: 'run_shell_command', args: { command: 'echo hi && ls' } }, undefined);
    ...
    it('should block unauthorized chained command with ||', async () => {
        ...
        const result = await engine.check({ name: 'run_shell_command', args: { command: 'false || ls' } }, undefined);
        expect(result.decision).toBe(PolicyDecision.ASK_USER);
    });
    it('should block unauthorized chained command with ;', async () => {
        engine = setupEngine(['echo']);
    ...
    it('should block unauthorized command in pipe |', async () => {
        engine = setupEngine(['echo']);
        const result = await engine.check({ name: 'run_shell_command', args: { command: 'echo hi | grep "hi"' } }, undefined);
        expect(result.decision).toBe(PolicyDecision.ASK_USER);
    });
    ...
    it('should block unauthorized command in nested substitution', async () => {
        ...
        const result = await engine.check({ name: 'run_shell_command', args: { command: 'echo $(cat $(ls))' } }, undefined);
        expect(result.decision).toBe(PolicyDecision.ASK_USER);
    });
    ...
    it('should block command redirection if not explicitly allowed', async () => {
        engine = setupEngine(['echo']);
        const result = await engine.check({ name: 'run_shell_command', args: { command: 'echo hi > /tmp/test' } }, undefined);
        // Inherent policy: redirection downgrades to ASK_USER
        expect(result.decision).toBe(PolicyDecision.ASK_USER);
    });
});
//# sourceMappingURL=shell-safety-regression.test.js.map