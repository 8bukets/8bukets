...
describe('createPolicyEngineConfig', () => {
    ...
    it('should NOT automatically allow configured MCP servers in non-interactive mode by default', async () => {
        ...
        }, ApprovalMode.DEFAULT, MOCK_DEFAULT_DIR, false);
        const rule = config.rules?.find((r) => r.mcpName === 'server-1' && r.decision === PolicyDecision.ALLOW);
    ...
    it('should automatically allow configured MCP servers in non-interactive mode if opted-in', async () => {
        ...
        const rule1 = config.rules?.find((r) => r.mcpName === 'server-1' && r.decision === PolicyDecision.ALLOW);
        const rule2 = config.rules?.find((r) => r.mcpName === 'server-2' && r.decision === PolicyDecision.ALLOW);
    ...
    it('should NOT automatically allow configured MCP servers in interactive mode even if opted-in', async () => {
        ...
        }, ApprovalMode.DEFAULT, MOCK_DEFAULT_DIR, true);
        const rule = config.rules?.find((r) => r.mcpName === 'server-1' && r.decision === PolicyDecision.ALLOW);
        expect(rule).toBeUndefined();
    });
    it('should NOT duplicate allow rules if an MCP server is already explicitly allowed, wildcard allowed, or trusted', async () => {
        ...
        const rules1 = config.rules?.filter((r) => r.mcpName === 'server-1' && r.decision === PolicyDecision.ALLOW);
        ...
        const rules2 = config.rules?.filter((r) => r.mcpName === 'server-2' && r.decision === PolicyDecision.ALLOW);
        expect(rules2).toHaveLength(0);
        // server-3: already trusted
        const rules3 = config.rules?.filter((r) => r.mcpName === 'server-3' && r.decision === PolicyDecision.ALLOW);
        ...
        const rules4 = config.rules?.filter((r) => r.mcpName === 'server-4' && r.decision === PolicyDecision.ALLOW);
        expect(rules4).toHaveLength(0);
        // Verify the wildcard rule exists
        const wildcardRule = config.rules?.find((r) => r.mcpName === '*' && r.decision === PolicyDecision.ALLOW);
    ...
    it('should use correct tool name pattern for wildcard server in headless auto-allow', async () => {
        ...
        const rule = config.rules?.find((r) => r.mcpName === '*' && r.decision === PolicyDecision.ALLOW);
        expect(rule).toBeDefined();
