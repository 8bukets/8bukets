...
import { EDIT_TOOL_NAME, GLOB_TOOL_NAME, GREP_TOOL_NAME, LS_TOOL_NAME, READ_FILE_TOOL_NAME, SHELL_TOOL_NAME, WRITE_FILE_TOOL_NAME, } from '../tools/tool-names.js';
...
describe('SkillExtractionAgent', () => {
    ...
    it('should require all memory updates to go through .inbox/<kind>/*.patch for review', () => {
        const prompt = SkillExtractionAgent(skillsDir, sessionIndex, existingSkillsSummary, '/tmp/memory').promptConfig.systemPrompt;
        expect(prompt).toContain('ALL memory updates are expressed as unified diff `.patch` files');
        ...
        expect(prompt).toContain('REWRITE that file by combining its existing hunks with your new');
        expect(prompt).toContain('private  ->');
        expect(prompt).toContain('global   ->');
        expect(prompt).toContain('the target MUST be exactly the single global personal memory');
        expect(prompt).toContain('~/.gemini/GEMINI.md');
        expect(prompt).not.toContain('memory.md');
        expect(prompt).not.toContain('and siblings');
        expect(prompt).toContain('Project/workspace shared instructions (GEMINI.md and similar files');
        ...
        expect(prompt).toContain('when you create a new sibling .md file, your patch SHOULD');
    ...
    it('surfaces existing inbox patches in the initial query when present', () => {
        ...
        const agentWithInbox = SkillExtractionAgent(skillsDir, sessionIndex, existingSkillsSummary, '/tmp/memory', pendingInbox);
    ...
    it('omits the pending inbox section when nothing is pending', () => {
        const agentEmpty = SkillExtractionAgent(skillsDir, sessionIndex, existingSkillsSummary, '/tmp/memory', '');
    ...
    it('should warn that session summaries are user-intent summaries, not workflow evidence', () => {
        ...
        expect(query).toContain('workflow hints alone is never enough evidence for a reusable skill.');
        expect(query).toContain('Session summaries describe user intent; optional workflow hints describe likely procedural traces.');
        ...
        expect(query).toContain('Only write memory if it would clearly help a future session.');
        expect(query).toContain('If recurrence, durability, or future reuse is unclear, create no artifact and explain why.');
    });
});
