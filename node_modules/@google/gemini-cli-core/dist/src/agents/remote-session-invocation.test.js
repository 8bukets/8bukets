...
function setupMockSession(options = {}) {
    ...
    }, error, progress, sessionState = {}, } = options;
    ...
    vi.mocked(RemoteSubagentSession).mockImplementation(() => mockSession);
    return {
        mockSession,
        subscriberCallbacks,
...
describe('RemoteSessionInvocation', () => {
    ...
    beforeEach(() => {
        ...
        RemoteSessionInvocation.sessionState?.clear();
    });
    afterEach(() => {
        vi.restoreAllMocks();
    });
    ...
    describe('Constructor Validation', () => {
        it('accepts valid input with string query', () => {
            expect(() => {
                new RemoteSessionInvocation(mockDefinition, mockContext, { query: 'hello' }, mockMessageBus);
            }).not.toThrow();
        });
        ...
        it('throws if query is not a string', () => {
            expect(() => {
                new RemoteSessionInvocation(mockDefinition, mockContext, { query: 123 }, mockMessageBus);
        ...
        it('throws if A2AClientManager is not available', () => {
            ...
            expect(() => {
                new RemoteSessionInvocation(mockDefinition, noA2AContext, { query: 'hi' }, mockMessageBus);
    ...
    describe('Execution Logic', () => {
        it('should create session and return result', async () => {
            ...
            const invocation = new RemoteSessionInvocation(mockDefinition, mockContext, { query: 'do stuff' }, mockMessageBus);
        ...
        it('should pass initial state from static map to session', async () => {
            ...
            RemoteSessionInvocation.sessionState.set('test-agent::http://test-agent/card', priorState);
            setupMockSession();
            const invocation = new RemoteSessionInvocation(mockDefinition, mockContext, { query: 'hi' }, mockMessageBus);
            await invocation.execute({
                abortSignal: new AbortController().signal,
            });
            // Verify the session constructor received the prior state
            expect(RemoteSubagentSession).toHaveBeenCalledWith(mockDefinition, mockContext, mockMessageBus, priorState);
        });
        it('should persist session state in finally block', async () => {
            ...
            const storedState = RemoteSessionInvocation.sessionState.get('test-agent::http://test-agent/card');
        ...
        it('should persist session state across invocations', async () => {
            ...
            const invocation1 = new RemoteSessionInvocation(mockDefinition, mockContext, { query: 'first' }, mockMessageBus);
            ...
            const invocation2 = new RemoteSessionInvocation(mockDefinition, mockContext, { query: 'second' }, mockMessageBus);
        ...
        it('should handle abort gracefully', async () => {
            ...
            expect(result.returnDisplay.recentActivity[0].status).toBe(SubagentState.CANCELLED);
    ...
    describe('Error Handling', () => {
        it('should handle execution errors gracefully', async () => {
            ...
            expect(result.returnDisplay.result).toContain('Network failure');
            // updateOutput should be called with error progress
            expect(updateOutput).toHaveBeenCalledWith(expect.objectContaining({ state: 'error' }));
        });
        ...
        it('should clean up listeners in finally', async () => {
            ...
            const invocation = new RemoteSessionInvocation(mockDefinition, mockContext, { query: 'hi' }, mockMessageBus, { onAgentEvent });
            await invocation.execute({
                abortSignal: controller.signal,
            });
            ...
            const postExecuteUnsubscribes = mockSession.subscribe.mock.results.map((r) => r.value);
    ...
    describe('SessionState Management', () => {
        it('should use composite name::url as session state key', async () => {
            ...
            const inv1 = new RemoteSessionInvocation(mockDefinition, mockContext, { query: 'hi' }, mockMessageBus);
            await inv1.execute({ abortSignal: new AbortController().signal });
            // Second agent
            setupMockSession({
                sessionState: { contextId: 'ctx-b' },
            });
            const inv2 = new RemoteSessionInvocation(secondDefinition, mockContext, { query: 'hi' }, mockMessageBus);
            await inv2.execute({ abortSignal: new AbortController().signal });
            const stateMap = RemoteSessionInvocation.sessionState;
        ...
        it('should isolate same-name agents with different URLs', async () => {
            ...
            const invA = new RemoteSessionInvocation(defA, mockContext, { query: 'hi' }, mockMessageBus);
            ...
            const invB = new RemoteSessionInvocation(defB, mockContext, { query: 'hi' }, mockMessageBus);
        ...
        it('should fall back to name-only key when URL is unavailable', async () => {
            ...
            const inv = new RemoteSessionInvocation(noUrlDef, mockContext, { query: 'hi' }, mockMessageBus);
...
//# sourceMappingURL=remote-session-invocation.test.js.map