...
import { AgentTerminateMode, SubagentActivityErrorType, SUBAGENT_REJECTED_ERROR_PREFIX, SUBAGENT_CANCELLED_ERROR_MESSAGE, } from './types.js';
...
function setupMockSession(config) {
    const mockSession = {
        ...
        getResult: config.error
            ...
            : vi.fn().mockResolvedValue(config.output ?? {
                result: 'done',
    ...
    MockLocalSubagentSession.mockImplementation((_def, _ctx, _bus, rawCallback) => {
...
describe('LocalSessionInvocation', () => {
    ...
    it('should pass the messageBus to the parent constructor', () => {
        ...
        const invocation = new LocalSessionInvocation(testDefinition, mockContext, params, mockMessageBus);
    ...
    describe('getDescription', () => {
        ...
        it('should not truncate long input values', () => {
            ...
            const invocation = new LocalSessionInvocation(testDefinition, mockContext, params, mockMessageBus);
            const description = invocation.getDescription();
            expect(description).toBe(`Running subagent 'MockAgent' with inputs: { task: ${'A'.repeat(100)} }`);
        });
        it('should not truncate the overall description', () => {
            ...
            const invocation = new LocalSessionInvocation(longNameDef, mockContext, params, mockMessageBus);
            const description = invocation.getDescription();
    ...
    describe('execute', () => {
        it('should create session and run successfully', async () => {
            ...
            expect(MockLocalSubagentSession).toHaveBeenCalledWith(testDefinition, mockContext, mockMessageBus, expect.any(Function));
        ...
        it('should stream THOUGHT_CHUNK activity', async () => {
            ...
            const progressCalls = updateOutput.mock.calls.map((c) => c[0]);
            const hasThought = progressCalls.some((p) => p.recentActivity &&
                p.recentActivity.some((a) => a.type === 'thought' && a.content === 'Analyzing...'));
        ...
        it('should stream TOOL_CALL_START and TOOL_CALL_END', async () => {
            ...
            const firstToolCall = runningCalls.find((p) => p.recentActivity?.some((a) => a.type === 'tool_call' && a.content === 'ls'));
            expect(firstToolCall).toBeDefined();
            // After TOOL_CALL_END, the tool should be completed
            const hasCompleted = progressCalls.some((p) => p.recentActivity?.some((a) => a.type === 'tool_call' &&
                a.content === 'ls' &&
                a.status === 'completed'));
        ...
        it('should handle ERROR activity', async () => {
            ...
            await executePromise;
            const progressCalls = updateOutput.mock.calls.map((c) => c[0]);
            const hasError = progressCalls.some((p) => p.recentActivity?.some((a) => a.type === 'thought' &&
        ...
        it('should handle cancelled errors', async () => {
            ...
            await executePromise;
            const progressCalls = updateOutput.mock.calls.map((c) => c[0]);
            const hasCancelled = progressCalls.some((p) => p.recentActivity?.some((a) => a.type === 'thought' && a.status === 'cancelled'));
        ...
        it('should handle rejected errors', async () => {
            ...
            const hasToolCancelled = progressCalls.some((p) => p.recentActivity?.some((a) => a.type === 'tool_call' &&
        ...
        it('should trim recentActivity to MAX_RECENT_ACTIVITY', async () => {
            ...
            const afterFourthActivity = progressCalls.filter((p) => p.state === 'running' && p.recentActivity.length > 0);
        ...
        it('should unsubscribe parent observer in finally', async () => {
            ...
            const invocation = new LocalSessionInvocation(testDefinition, mockContext, params, mockMessageBus, { onAgentEvent });
            await invocation.execute({ abortSignal: signal, updateOutput });
        ...
        it('should handle TOOL_CALL_END with error data', async () => {
            ...
            await executePromise;
            const progressCalls = updateOutput.mock.calls.map((c) => c[0]);
            const hasToolError = progressCalls.some((p) => p.recentActivity?.some((a) => a.type === 'tool_call' &&
        ...
        it('should mark running items as cancelled on abort', async () => {
            ...
            expect(lastProgress.recentActivity).toContainEqual(expect.objectContaining({
                type: 'tool_call',
...
//# sourceMappingURL=local-session-invocation.test.js.map