...
vi.mock('../config/scoped-config.js', async (importOriginal) => {
    ...
    return {
        ...
        runWithScopedAutoMemoryExtractionWriteAccess: vi.fn(actual.runWithScopedAutoMemoryExtractionWriteAccess),
        runWithScopedMemoryInboxAccess: vi.fn(actual.runWithScopedMemoryInboxAccess),
    };
});
import { runWithScopedWorkspaceContext, createScopedWorkspaceContext, runWithScopedAutoMemoryExtractionWriteAccess, runWithScopedMemoryInboxAccess, } from '../config/scoped-config.js';
...
const mockedRunWithScopedAutoMemoryExtractionWriteAccess = vi.mocked(runWithScopedAutoMemoryExtractionWriteAccess);
...
describe('LocalAgentExecutor', () => {
    ...
    describe('create (Initialization and Validation)', () => {
        ...
        it('should not include parentCallId in agentId even when available', async () => {
            ...
            const executor = await runWithToolCallContext({ callId: parentCallId, schedulerId: 'test-scheduler' }, () => LocalAgentExecutor.create(definition, mockConfig, onActivity));
            expect(executor['agentId']).not.toContain(parentCallId);
            expect(executor['agentId']).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i);
        });
        it('should correctly apply templates to initialMessages', async () => {
            ...
            const firstPart = 'content' in history[0]
                ? history[0].content.parts?.[0]
                : history[0].parts?.[0];
            expect(firstPart?.text).toBe('Goal: TestGoal');
            const secondPart = 'content' in history[1]
    ...
    describe('run (Execution Loop and Logic)', () => {
        ...
        it('should inject Plan Mode context into the system prompt when in Plan Mode', async () => {
            ...
            expect(systemInstruction).toContain('You are currently operating in Plan Mode. Your write tools are globally restricted to only modifying plan (.md) files in the plans directory: /mock/plans/');
        });
    ...
    describe('Edge Cases and Error Handling', () => {
        ...
        it('should throw a critical error when a tool response is dropped by the scheduler', async () => {
            ...
            await expect(executor.run({ goal: 'Protocol test' }, signal)).rejects.toThrow('Critical System Failure: Tool execution result was lost/dropped by the scheduler');
        });
        it('should throw a critical error when all scheduler results are missing/dropped', async () => {
            ...
            mockScheduleAgentTools.mockResolvedValueOnce([]);
            await expect(executor.run({ goal: 'Protocol test 2' }, signal)).rejects.toThrow('Critical System Failure: Tool execution result was lost/dropped by the scheduler');
        });
    });
    describe('Model Routing', () => {
        ...
        it('should cache the routing decision across multiple turns', async () => {
            ...
            expect(mockSendMessageStream).toHaveBeenCalledTimes(2);
            expect(mockSendMessageStream).toHaveBeenNthCalledWith(1, expect.objectContaining({ model: 'routed-model' }), expect.any(Array), expect.any(String), expect.any(AbortSignal), LlmRole.SUBAGENT);
            expect(mockSendMessageStream).toHaveBeenNthCalledWith(2, expect.objectContaining({ model: 'routed-model' }), expect.any(Array), expect.any(String), expect.any(AbortSignal), LlmRole.SUBAGENT);
        });
    ...
    describe('Chat Compression', () => {
        ...
        it('should update chat history when compression is successful', async () => {
            ...
            expect(mockSetHistory).toHaveBeenCalledWith(expect.arrayContaining([
                expect.objectContaining({
        ...
        describe('Memory Injection', () => {
            ...
            it('should omit extension context from session memory when disabled by the agent', async () => {
                ...
                const getSessionMemorySpy = vi
                    ...
                    .mockImplementation((options) => options?.includeExtensionContext === false
                    ? '<loaded_context>\n<project_context>\nProject memory rule\n</project_context>\n</loaded_context>'
                    : '<loaded_context>\n<extension_context>\nExtension memory rule\n</extension_context>\n<project_context>\nProject memory rule\n</project_context>\n</loaded_context>');
                mockModelResponse([
                    {
