...
describe('AgentTool', () => {
    ...
    describe('agentSessionSubagentEnabled feature flag', () => {
        it('should use LocalSessionInvocation when flag is enabled for local agent', async () => {
            ...
            const invocation = tool['createInvocation'](params, mockMessageBus);
            await invocation.shouldConfirmExecute(new AbortController().signal);
            expect(LocalSessionInvocation).toHaveBeenCalledWith(testLocalDefinition, mockConfig, { objective: 'Do something' }, mockMessageBus, undefined);
        ...
        it('should use RemoteSessionInvocation when flag is enabled for remote agent', async () => {
            ...
            const invocation = tool['createInvocation'](params, mockMessageBus);
            await invocation.shouldConfirmExecute(new AbortController().signal);
            expect(RemoteSessionInvocation).toHaveBeenCalledWith(testRemoteDefinition, mockConfig, { query: 'Search something' }, mockMessageBus, undefined);
        ...
        it('should thread onAgentEvent to session invocations', async () => {
            ...
            const invocation = tool['createInvocation'](params, mockMessageBus);
            await invocation.shouldConfirmExecute(new AbortController().signal);
            expect(LocalSessionInvocation).toHaveBeenCalledWith(testLocalDefinition, mockConfig, { objective: 'Do something' }, mockMessageBus, { onAgentEvent: onEvent });
        });
