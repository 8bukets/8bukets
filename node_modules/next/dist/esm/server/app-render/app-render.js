 ...
 */ async function validateInstantConfigs(accumulatedChunks, debugChunks, startTime, rootParams, fallbackRouteParams, ctx, hmrRefreshHash, validationSamples) {
    ...
    async function validateAtDepthImpl(depth, groupDepthForValidation, previousBoundaryState) {
        ...
        try {
            ...
            const { preludeIsEmpty } = await processPreludeOp(unprocessedPrelude);
            result = getNavigationDisallowedDynamicReasons(workStore, preludeIsEmpty ? PreludeState.Empty : PreludeState.Full, instantValidationState, validationSampleTracking, boundaryState);
        } catch (thrownValue) {
            result = getNavigationDisallowedDynamicReasons(workStore, PreludeState.Errored, instantValidationState, validationSampleTracking, boundaryState);
        }
