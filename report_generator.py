...
class ReportGenerator:
    ...
    def generate_daily_report(self):
        ...
        with open(report_filename, "w", encoding="utf-8") as f:
            f.write(f"# <a id='top'></a>Daily Scraper Report - {report_date}\n\n")
            ...
            f.write("## <a id='recommendations'></a>💡 Recommendations\n\n")
            ...
            if all_recent_titles:
                f.write("## <a id='keyword-trends'></a>🧠 Keyword Trends\n\n")
            ...
            # SEO Rankings Trend
            f.write("## <a id='seo-trend-analysis'></a>📈 SEO Trend Analysis\n\n")
            if rankings:
            ...
            if updated_posts:
                f.write("## <a id='content-updates'></a>🔄 Content Updates\n\n")

                if len(updated_posts) > 5:
                    f.write(f"<details>\n<summary>View {len(updated_posts)} Updates</summary>\n\n")
                ...
                for u in updated_posts:
                    ...
                    # Truncate then escape to avoid breaking entities
                    old = str(old)
                    new = str(new)
                    old_display = (old[:50] + '...') if len(old) > 50 else old
                    new_display = (new[:50] + '...') if len(new) > 50 else new
            ...
            # New Posts Section
            if new_posts:
                f.write("## <a id='new-posts'></a>🆕 Recently Scraped Posts\n\n")

                if len(new_posts) > 5:
                    f.write(f"<details>\n<summary>View {len(new_posts)} New Posts</summary>\n\n")
