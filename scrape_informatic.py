...
def scrape(output_file: str, max_pages: int = 0):
    ...
    while current_url:
        ...
        # Optimization: Use SoupStrainer to only parse 'article' (posts) and
        # 'nav' (pagination) tags. This reduces parsing time by skipping
        # unrelated tags like scripts, styles, and generic containers.
        parse_only = SoupStrainer(['article', 'nav'])
        soup = BeautifulSoup(
            response.content, 'html.parser', parse_only=parse_only)
