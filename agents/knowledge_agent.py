        ...
        try:
            ...
            definitions = {
                ...
                "benefits": "\n\n".join([
                    ...
                    knowledge.get("what-is-the-difference-between-ai-agents-ai-assistants-and-bots", {}).get("content", ""),
            ...
            if tools_content:
                ...
                for line in tools_content.split("\n"):
                    if line.startswith("- "):
                        ...
                        # Improved to handle descriptions starting immediately after the name
                        match = re.search(r"^- ([\w\s\(\)-]{2,60}?)(?::|\s+[A-Z]|$)", line)
                        if match:
                            tool_name = match.group(1).strip()
                            if tool_name and len(tool_name.split()) <= 6:
                                tools_list.append(tool_name)
                        elif ":" in line:
                            tool_name = line.split(":", 1)[0].replace("- ", "").strip()
                            if tool_name and len(tool_name.split()) <= 6:
                                tools_list.append(tool_name)
