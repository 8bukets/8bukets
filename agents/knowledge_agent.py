            ...
            if tools_content:
                ...
                for line in tools_content.split("\n"):
                    if line.startswith("- "):
                        ...
                        match = re.search(r"^- ([\w\s\(\)-]{1,60}?)(?:\s+(?:Secure platform|Create AI|Build hybrid|Build Google-quality|Curated collection|Open-source|An AI|A fully managed|Provides a|Unified|Single|End-to-end|Speech|Language|Custom|Omnichannel|Description)|AppSecure platform|Create AI|StudioBuild hybrid|GardenCurated collection|Kit \(ADK\)Open-source|ProtocolAn open-source|RunA fully managed|$)", line)
                        if match:
