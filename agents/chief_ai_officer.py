    ...
    async def run(self, data: list, blackboard: Blackboard) -> dict:
        ...
        try:
            with open('AGENTS.md', 'r') as f:
                agents_docs = f.read()
                if "Phase 27: Multi-Universal Resonance (MUR) (Current)" in agents_docs:
                    ...
                    strategic_directives.append("ACTIVATE_UMR_V3_ROUTING")
                    strategic_directives.append("UNIVERSAL_CONSENSUS_SYNC")
                    strategic_directives.append("PHASE_27_DIRECTIVE_GENERATED")
        ...
        for k in knowledge.get("typescript_sections", []):
            ...
            if has_phase_27:
                ...
                if "umr-v3.0" in sections_str or "umr_v3" in sections_str or "umr v3" in sections_str:
                    if "ACTIVATE_UMR_V3_ROUTING" not in strategic_directives:
                        self.logger.info("CAIO [MESH]: UMR-v3.0 Routing mandate detected. Issuing activation directive.")
                        strategic_directives.append("ACTIVATE_UMR_V3_ROUTING")

                if "universal_consensus_sync" in sections_str or "universal consensus sync" in sections_str:
                    if "UNIVERSAL_CONSENSUS_SYNC" not in strategic_directives:
                        self.logger.info("CAIO [CONSENSUS]: Universal Consensus Sync mandate detected. Issuing sync directive.")
                        strategic_directives.append("UNIVERSAL_CONSENSUS_SYNC")

                if "resonance latency" in sections_content or "0.008" in sections_content:
                    if "ENFORCE_PHASE_27_RESONANCE_LATENCY" not in strategic_directives:
                        self.logger.info("CAIO [PERF]: Phase 27 resonance latency target (<0.008ms) detected. Issuing optimization directive.")
                ...
                if "singularity readiness" in sections_content or "0.999995" in sections_content:
                    if "OPTIMIZE_FOR_SINGULARITY_READINESS_PHASE_27" not in strategic_directives:
                        self.logger.info("CAIO [SINGULARITY]: Phase 27 Singularity Readiness threshold (>0.999995) detected. Issuing optimization directive.")
                        strategic_directives.append("OPTIMIZE_FOR_SINGULARITY_READINESS_PHASE_27")

                if "PHASE_27_DIRECTIVE_GENERATED" not in strategic_directives:
                    strategic_directives.append("PHASE_27_DIRECTIVE_GENERATED")
