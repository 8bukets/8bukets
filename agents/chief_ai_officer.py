...
class ChiefAIOfficerAgent(BaseAgent):
    ...
    async def run(self, data: list, blackboard: Blackboard) -> dict:
        ...
        for k in knowledge.get("typescript_sections", []):
            ...
            has_phase_27 = any(p in title_lower or p in sections_str for p in ["phase 27", "phase_27"])
            ...
            if has_phase_27:
                ...
                if "dni hooks" in sections_str or "dni_hooks" in sections_str:
                    if "INITIALIZE_DNI_HOOKS" not in strategic_directives:
                        self.logger.info("CAIO [SINGULARITY]: DNI Hooks mandate detected. Issuing initialization directive.")
                        strategic_directives.append("INITIALIZE_DNI_HOOKS")
                ...
                if "resonance latency" in sections_content or "0.01ms" in sections_content:
        ...
        if "OPTIMIZE_FOR_SINGULARITY_READINESS_PHASE_27" in strategic_directives:
            self.logger.info("CAIO [SINGULARITY]: Mandating high-frequency Phase 27 Singularity Readiness audits (6h cycle).")
