    ...
    async def run(self, data: list, blackboard: Blackboard) -> dict:
        ...
        try:
            with open('AGENTS.md', 'r') as f:
                agents_docs = f.read()
                if "Phase 27: Multi-Universal Resonance (Current)" in agents_docs or "Phase 27" in agents_docs or "MUR" in agents_docs:
            ...
            if has_phase_27:
                ...
                if "dni" in sections_str or "direct neural interface" in sections_str:
                    if "INITIALIZE_DNI_HOOKS" not in strategic_directives:
                        self.logger.info("CAIO [SYNC]: DNI mandate detected. Issuing initialization directive.")
                        strategic_directives.append("INITIALIZE_DNI_HOOKS")

                if "universal-consensus" in sections_str or "universal consensus" in sections_str:
                    if "ENFORCE_UNIVERSAL_CONSENSUS" not in strategic_directives:
                        self.logger.info("CAIO [SYNC]: Universal Consensus mandate detected. Issuing enforcement directive.")
                        strategic_directives.append("ENFORCE_UNIVERSAL_CONSENSUS")

                if "resonance latency" in sections_str or "0.008ms" in sections_str:
                ...
                if "singularity readiness" in sections_str or "0.999995" in sections_str:
