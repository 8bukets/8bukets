...
async function scrapeMarkpositionKnowledge(maxPages: number = 5) {
    console.log(`🤖 [Ingest] Fetching market intelligence from ${BASE_URL} (Max Pages: ${maxPages})...`);
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    console.log(` ⏹️ Page ${page} not found. Ending pagination.`);
                    break;
                }
            ...
            if (pageEntries.length === 0) {
                console.log(` ⏹️ No entries found on page ${page}. Ending pagination.`);
                break;
            }

            console.log(` ✅ Parsed ${pageEntries.length} entries from page ${page}.`);
    ...
    console.log(`✅ [Ingest] Total Parsed: ${allEntries.length} entries.`);
