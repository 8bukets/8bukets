...
class MarkPositionScraperAsync:
    ...
    def clean_text(self, text: str) -> str:
        ...
        if not text:
            return ""
        # \s matches \xa0 (non-breaking space) in Python 3, so explicit replacement is not needed
        # when we use the regex substitution.
                ...
                for task in done:
                    try:
                        ...
                        if page_posts is None:
                            # 404 or Error
                            logger.info(
                                f"Page {page_num} returned 404 or error. Stopping new scheduling."
                            )
                            stop_scheduling = True
                        elif len(page_posts) == 0:
                            logger.info(
                                f"Page {page_num} has no articles. Stopping new scheduling."
                            )
                            stop_scheduling = True
                        else:
                            logger.info(
                                f"Page {page_num} scraped ({len(page_posts)} posts)."
                            )
