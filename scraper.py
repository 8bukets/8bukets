...
class MarkPositionScraperAsync:
    ...
    async def parse_page(self, html: str) -> List[Dict]:
        """Parse HTML using module-level function. Kept for compatibility."""
    ...
    async def scrape(self):
        ...
        # Limit workers to avoid too much overhead, default is usually number of processors
    ...
    async def fetch_and_parse(self, session, page_num, sem, pool):
        async with sem:
            ...
            if html:
                # Offload parsing to ProcessPoolExecutor
                loop = asyncio.get_running_loop()
                return await loop.run_in_executor(pool, parse_page_html, html)
            return None
