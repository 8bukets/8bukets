...
class MarkPositionScraperAsync:
    ...
    async def scrape(self):
        ...
        # We limit max_workers to avoid too much overhead, but default is fine usually
        with concurrent.futures.ProcessPoolExecutor() as executor:
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                while len(tasks) < self.concurrency and not stop_scheduling:
                    ...
                    # Create task
                    task = asyncio.create_task(self.fetch_and_parse(session, next_page, executor))
                    tasks.add(task)
                    next_page += 1
    ...
    async def fetch_and_parse(self, session, page_num, executor) -> Tuple[int, Optional[List[Dict]]]:
        html = await self.fetch_page(session, page_num)
        if html:
            loop = asyncio.get_running_loop()
            # Run CPU-bound parsing in the executor
            posts = await loop.run_in_executor(executor, parse_page, html)
