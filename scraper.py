...
def parse_html(html: str) -> List[Dict]:
    ...
    This function is CPU-bound and should be run in a separate process.
    """
    # Optimization: Use SoupStrainer to only parse article tags with 'post' in class
    # This avoids parsing the entire DOM tree (header, footer, sidebar)
    strainer = SoupStrainer('article', attrs={'class': re.compile(r'\bpost\b')})
    soup = BeautifulSoup(html, 'html.parser', parse_only=strainer)

    # Note: When using parse_only, soup object behaves slightly differently depending on structure,
    # but find_all should work on the parsed fragments.
...
class MarkPositionScraperAsync:
    ...
    async def fetch_and_parse(self, session, page_num, sem):
        async with sem:
            ...
            if html:
                # Run CPU-bound parsing in a separate process
                loop = asyncio.get_running_loop()
                return await loop.run_in_executor(self.executor, parse_html, html)
            return None
