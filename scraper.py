...
class MarkPositionScraperAsync:
    ...
    def sanitize_csv_field(self, field: str) -> str:
        ...
        if not field:
            return ""
        # If the field starts with potential formula characters, prepend a single quote
        if field.startswith(('=', '+', '-', '@')):
            return f"'{field}"
        return field
    ...
    def save_data(self, posts: List[Dict]):
        ...
        try:
            with open(self.output_csv, 'w', newline='', encoding='utf-8') as f:
                ...
                for post in posts:
                    # Join categories before sanitizing the whole field string
                    categories_str = ", ".join(post.get('categories', []))

                    writer.writerow([
                        self.sanitize_csv_field(post.get('title', '')),
                        self.sanitize_csv_field(post.get('date', '')),
                        self.sanitize_csv_field(post.get('author', '')),
                        self.sanitize_csv_field(categories_str),
