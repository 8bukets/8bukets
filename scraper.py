...
def parse_date(date_text: str) -> Optional[Dict[str, str]]:
...
def process_page(html: str) -> List[Dict]:
    """
    Parses the page HTML to extract news posts.
    Uses Regex to locate the hidden news section in comments for performance,
    then parses that section with BeautifulSoup.
    """
    # Use Regex to find hidden news section in HTML comments
    # This avoids parsing the full page DOM with BeautifulSoup which is slow.
    news_html = None
    # Look for comment containing rc92v0 and <section
    # Using finditer to be memory efficient and robust
    ...
    for article in articles:
        ...
        date_text = clean_text(date_tag.get_text()) if date_tag else ""
...
class OracleNewsScraper:
    ...
    async def scrape(self):
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            if html:
                # Offload parsing to thread to unblock event loop
                posts = await asyncio.to_thread(process_page, html)
