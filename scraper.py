...
class ColoredFormatter(logging.Formatter):
    def format(self, record):
        if record.levelno == logging.INFO:
            record.msg = colorize(record.msg, Colors.BLUE)
        elif record.levelno == logging.WARNING:
            record.msg = colorize(record.msg, Colors.WARNING)
        elif record.levelno == logging.ERROR:
            record.msg = colorize(record.msg, Colors.FAIL)
        elif record.levelno == logging.CRITICAL:
            record.msg = colorize(record.msg, Colors.FAIL + Colors.BOLD)
...
class MarkPositionScraperAsync:
    ...
    async def scrape(self):
        print(colorize("\n🚀 Starting Scraper...", Colors.HEADER))
        all_posts = []
        page_num = 1
    ...
    def print_summary(self, posts):
        ...
        print(colorize("║", Colors.CYAN) + colorize(" Scrape Summary".center(50), Colors.BOLD + Colors.HEADER) + colorize("║", Colors.CYAN))
        print(colorize("╠" + "═"*50 + "╣", Colors.CYAN))
        print(colorize("║", Colors.CYAN) + f" Total Posts:     {len(posts)}".ljust(50) + colorize("║", Colors.CYAN))
        print(colorize("║", Colors.CYAN) + f" Unique Links:    {unique_links}".ljust(50) + colorize("║", Colors.CYAN))
        print(colorize("║", Colors.CYAN) + f" JSON Output:     {self.output_json}".ljust(50) + colorize("║", Colors.CYAN))
        print(colorize("║", Colors.CYAN) + f" CSV Output:      {self.output_csv}".ljust(50) + colorize("║", Colors.CYAN))
        print(colorize("║", Colors.CYAN) + f" TXT Output:      {self.output_txt}".ljust(50) + colorize("║", Colors.CYAN))
        print(colorize("╚" + "═"*50 + "╝", Colors.CYAN))
        print("\n" + colorize("✨ Done!", Colors.GREEN + Colors.BOLD))
