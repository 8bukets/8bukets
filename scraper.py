...
class MarkPositionScraperAsync:
    # ⚡ Bolt Optimization: Pre-compile regex patterns to avoid repeated compilation overhead.
    ...
    def clean_text(self, text: str) -> str:
        ...
        if not text:
            return ""
        # ⚡ Bolt Optimization: split() and join() is ~6x faster than regex for whitespace normalization
        # and automatically handles \xa0 (non-breaking space) as whitespace.
        return " ".join(text.split())

    def is_url(self, text: str) -> bool:
        """Check if text looks like a URL."""
        # ⚡ Bolt: Use pre-compiled pattern
