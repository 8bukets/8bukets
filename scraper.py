...
def parse_page_sync(html: str) -> List[Dict]:
    """Standalone parsing function for ProcessPoolExecutor."""
...
class MarkPositionScraperAsync:
    ...
    async def parse_page(
        ...
        return await loop.run_in_executor(executor, parse_page_sync, html)

    async def fetch_and_parse(self, session, page_num, sem, executor):
        async with sem:
            html = await self.fetch_page(session, page_num)
            if html:
                return await self.parse_page(html, executor)
            return None
