...
class MarkPositionScraperAsync:
    ...
    async def parse_page(self, html: str) -> List[Dict]:
        # Optimize parsing by only parsing article elements with class 'post'
        # This reduces parsing time by ~3x
        strainer = SoupStrainer('article', class_='post')
        soup = BeautifulSoup(html, 'lxml', parse_only=strainer)

        # When using SoupStrainer, the soup object itself might be a list of tags or a partial tree.
        # find_all still works on the soup object.
