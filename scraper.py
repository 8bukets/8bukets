...
class MarkPositionScraperAsync:
    ...
    def sanitize_for_csv(self, text: str) -> str:
        """Prevent CSV Injection (Formula Injection) by prepending a single quote to dangerous characters."""
        if not text:
            return ""
        text = str(text)
        # If the text starts with =, +, -, or @, it can be interpreted as a formula in Excel/Sheets.
        if text.startswith(('=', '+', '-', '@')):
            return "'" + text
        return text
