...
def parse_html_content(html: str) -> List[Dict]:
    """
    Parses HTML content to extract posts.
    This function is CPU-bound and designed to run in a separate process.
    """
...
class MarkPositionScraperAsync:
    ...
    # Helper methods delegated to top-level functions for backward compatibility if needed,
    # or just used internally. Since they are used inside `parse_html_content` which is running
    # in another process, the instance methods are not used during parsing.
    # But I'll keep them as wrappers if other code relies on them, though likely not.
    # Actually, to keep code clean, I'll remove them from the class if they are not used elsewhere.
    # Based on my search, they are only used in `parse_page`.

    # I will keep `clean_text` etc. as static methods or just remove them from class interface
    # to avoid confusion. `scraper.py` is likely the only consumer.
    ...
    async def parse_page(self, html: str, loop: asyncio.AbstractEventLoop, executor: concurrent.futures.ProcessPoolExecutor) -> List[Dict]:
        """
        Offloads parsing to the process pool executor.
        """
        return await loop.run_in_executor(executor, parse_html_content, html)

    async def fetch_and_parse(self, session, page_num, sem, loop, executor):
        async with sem:
            html = await self.fetch_page(session, page_num)
            if html:
                return await self.parse_page(html, loop, executor)
            return None
    ...
    async def scrape(self):
        ...
        # Using concurrency as max_workers.
        # Note: If concurrency is high (e.g. 50), creating 50 processes is bad.
        # But default is 5.
        max_workers = min(self.concurrency, 8) # Cap workers to reasonable number for CPU tasks
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                while active:
                    ...
                    for i in range(current_concurrency):
                        ...
                        tasks.append(self.fetch_and_parse(session, current_page, sem, loop, executor))
