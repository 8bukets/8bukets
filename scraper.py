...
def parse_html_content(html: str) -> List[Dict]:
    """
    Parses HTML content to extract post data.
...
<<<<<<< bolt-scraper-optimization-8033040251280167060
    ...
    def sanitize_for_csv(self, text: str) -> str:
        ...
        if not text:
            return ""
        # If the text starts with a dangerous character, prepend a single quote
        if text.startswith(('=', '+', '-', '@')):
            return f"'{text}"
        return text
    ...
    def sanitize_for_csv(self, text: Optional[str]) -> str:
        ...
        if not text:
            return ""
        # If the text starts with any of the dangerous characters, prepend a single quote
        if text.startswith(('=', '+', '-', '@')):
            return f"'{text}"
        return text
        ...
        with ProcessPoolExecutor() as executor:
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                while active:
                    ...
                    logger.info(f"Fetching pages {batch_start} to "
                    ...
                    for idx, page_posts in enumerate(results):
                        ...
                        if page_posts is None:
                            # 404 or Error
                            logger.info(f"Page {page_idx} returned 404 or "
                                        "empty. Stopping.")
                            stop_detected = True
                            break
                        elif len(page_posts) == 0:
                            logger.info(f"Page {page_idx} has no articles. "
                                        "Stopping.")
                            stop_detected = True
                            break
                        else:
        ...
        try:
            with open(self.output_txt, 'w', encoding='utf-8') as f:
                for link in sorted_links:
                    f.write(link + '\n')
            logger.info(f"Saved {len(sorted_links)} unique links to "
                        f"{self.output_txt}")
        except IOError as e:
