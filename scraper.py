...
class MarkPositionScraperAsync:
    ...
    async def parse_page(self, html: str) -> List[Dict]:
        ...
        # Offload synchronous parsing to a thread to prevent blocking the event loop
        return await asyncio.to_thread(self._parse_page_sync, html)

    def _parse_page_sync(self, html: str) -> List[Dict]:
        """Synchronous part of page parsing using BeautifulSoup."""
    ...
    async def scrape(self):
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            # We don't know the total pages, so we have to fetch sequentially or in chunks
            # until we hit 404/empty.
            # Pure concurrent fetching of all pages requires knowing the max page.
            # Heuristic: fetch in batches of `concurrency`.
            # If any page in batch returns 404 or empty, stop.
            ...
            while active:
                ...
                logger.info("Fetching pages %s to %s...", batch_start, batch_start + len(tasks) - 1)
                ...
                for idx, page_posts in enumerate(results):
                    ...
                    if page_posts is None:
                        ...
                        stop_detected = True
                        break # Don't process further pages in this batch effectively
                    if len(page_posts) == 0:
    ...
    async def fetch_and_parse(self, session, page_num, sem):
        """Fetch and parse a page with semaphore."""
        async with sem:
    ...
    def save_data(self, posts: List[Dict]):
        ...
        try:
            ...
            logger.info("Saved %s posts to %s", len(posts), self.output_json)
        except IOError as e:
        ...
        try:
            with open(self.output_csv, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(['Title', 'Date', 'Author', 'Categories',
            ...
            logger.info("Saved %s posts to %s", len(posts), self.output_csv)
        except IOError as e:
        ...
        try:
            with open(self.output_txt, 'w', encoding='utf-8') as f:
                for link in sorted_links:
                    f.write(link + '\n')
            logger.info("Saved %s unique links to %s", len(sorted_links), self.output_txt)
        except IOError as e:
