...
class MarkPositionScraperAsync:
    ...
    async def parse_page(self, html: str) -> List[Dict]:
        ...
        for article in articles:
            ...
            # Title
            title_text = ""
            # Optimized: replace select_one('h1.entry-title a')
            h1_tag = article.find('h1', class_='entry-title')
            title_tag = h1_tag.find('a') if h1_tag else None
            ...
            # Date
            # Optimized: replace select_one('time.entry-date')
            ...
            # Author
            # Optimized: replace select_one('.author.vcard .fn')
            # Assuming .author.vcard is a single element with classes "author" and "vcard"
            ...
            # External Link
            external_link = None
            # Optimized: replace select_one('.entry-content')
            content_div = article.find(class_='entry-content')

            if content_div:
                # Optimized: replace select_one('a')
                link_tag = content_div.find('a')
                if link_tag:
                    external_link = link_tag.get('href')

                if not external_link:
                    # Optimized: replace select_one('iframe')
