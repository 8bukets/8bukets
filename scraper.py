...
def parse_page_content(html: str) -> List[Dict]:
    """Parse HTML content and extract posts data.

    This function is CPU-bound and should be run in a ProcessPoolExecutor.
    """
...
class MarkPositionScraperAsync:
    ...
    async def scrape(self):
        ...
        # Optimization: Offload BeautifulSoup parsing to separate processes to avoid blocking the asyncio loop.
    ...
    async def fetch_and_parse(self, session, page_num, sem):
        async with sem:
            ...
            if html:
                # Offload CPU-bound parsing to the process pool
                loop = asyncio.get_running_loop()
                return await loop.run_in_executor(self.executor, parse_page_content, html)
            return None
