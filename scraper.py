...
class MarkPositionScraperAsync:
    ...
    async def scrape(self):
        ...
        # Note: 'spawn' is default on Windows/MacOS, 'fork' on Linux.
        # Since we use module-level functions, it should work on all.
                ...
                while active:
                    ...
                    for i in range(current_concurrency):
                        ...
                        if self.max_pages and current_page > self.max_pages:
                            active = False
                            break
                        tasks.append(self.fetch_and_parse(session, executor, current_page, sem))
    ...
    async def fetch_and_parse(self, session, executor, page_num, sem):
        async with sem:
            html = await self.fetch_page(session, page_num)
            if html:
                loop = asyncio.get_running_loop()
                return await loop.run_in_executor(executor, parse_html, html)
            return None
