...
def parse_page_sync(html: str) -> List[Dict]:
    """
    Synchronous CPU-bound parsing of HTML content.
...
class MarkPositionScraperAsync:
    def __init__(self, output_json: str, output_csv: str, output_txt: str,
        ...
        self.session = None
        # Initialize executor for CPU-bound parsing
        # Default to number of processors on the machine
    ...
    async def parse_page(self, html: str) -> List[Dict]:
        """
        Offload parsing to a separate process to avoid blocking the event loop.
        """
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(self.executor, parse_page_sync, html)
            ...
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                while active:
                    ...
                    for i in range(current_concurrency):
                        ...
                        tasks.append(
                            self.fetch_and_parse(session, current_page, sem)
                        )
                    ...
                    logger.info(
                        f"Fetching pages {batch_start} to "
                        f"{batch_start + len(tasks) - 1}..."
                    )
                    ...
                    for idx, page_posts in enumerate(results):
                        ...
                        if page_posts is None:
                            # 404 or Error
                            logger.info(
                                f"Page {page_idx} returned 404 or empty. "
                                "Stopping."
                            )
                            stop_detected = True
                            break
                    ...
                    if self.max_pages and \
                            (batch_start + len(tasks) - 1) >= self.max_pages:
...
def main():
    ...
    parser.add_argument(
        "--json", default="links.json", help="Output JSON filename"
    )
    parser.add_argument(
        "--csv", default="links.csv", help="Output CSV filename"
    )
    parser.add_argument(
        "--txt", default="unique_links.txt",
        help="Output TXT filename for unique links"
    )
    parser.add_argument(
        "--limit", type=int, help="Limit number of pages to scrape"
    )
    parser.add_argument(
