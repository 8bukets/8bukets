...
# Helper functions moved to module level for pickling/multiprocessing support
...
def parse_page_content(html: str) -> List[Dict]:
    """Parse HTML content to extract posts (CPU-bound)."""
...
class MarkPositionScraperAsync:
        ...
        with concurrent.futures.ProcessPoolExecutor() as pool:
            async with aiohttp.ClientSession(headers=headers) as session:
                ...
                while len(tasks) < self.concurrency and not stop_scheduling:
                    ...
                    # Create task
                    task = asyncio.create_task(self.fetch_and_parse(session, pool, next_page))
                    tasks.add(task)
                    next_page += 1
    ...
    async def fetch_and_parse(self, session, pool, page_num) -> Tuple[int, Optional[List[Dict]]]:
        html = await self.fetch_page(session, page_num)
        if html:
            # Offload CPU-bound parsing to process pool
            loop = asyncio.get_running_loop()
            posts = await loop.run_in_executor(pool, parse_page_content, html)
