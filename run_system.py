"""
Orchestration script for the Autonomous Agents System.
Handles pipeline execution, agent instantiation, and result aggregation.
"""
import json
import os
import argparse
...
def run_pipeline(skip_scrape=False):
    ...
    logger.info(f"\n✅ Pipeline Complete for {current_date}. Check 'results/' directory.")
...
def main():
    ...
    if args.daemon:
        logger.info(
            f"Starting Daemon Mode. Running every {args.interval} seconds."
        )
        while True:
            try:
